...
void AdvectionScheme::backtrace(const Grid& g, double x, double y, double dt, double& xp,
                                double& yp) {
    ...
    for (int i = 1; i < nx; i++) {
        for (int j = 1; j <= ny; j++) {
            ...
            if (j > 1) {
                ...
                int cnt   = 0;
                double s  = 0;
                if (!g.is_solid(i, j - 1) && !g.is_solid(i, j)) {
                    s += v_old[g.iv(i, j - 1)];
                    cnt++;
                }
                if (i < nx && !g.is_solid(i + 1, j - 1) && !g.is_solid(i + 1, j)) {
                    s += v_old[g.iv(i + 1, j - 1)];
                    cnt++;
                }
                if (cnt > 0)
                    vB = s / cnt;
            ...
            if (j < ny) {
                ...
                if (i < nx && !g.is_solid(i + 1, j) && !g.is_solid(i + 1, j + 1)) {
                    s += v_old[g.iv(i + 1, j)];
                    cnt++;
                }
                if (cnt > 0)
    ...
    for (int i = 1; i <= nx; i++) {
        for (int j = 1; j < ny; j++) {
            ...
            if (i > 1) {
                ...
                int cnt   = 0;
                double s  = 0;
                if (!g.is_solid(i - 1, j) && !g.is_solid(i, j)) {
                    s += u_old[g.iu(i - 1, j)];
                    cnt++;
                }
                if (j < ny && !g.is_solid(i - 1, j + 1) && !g.is_solid(i, j + 1)) {
                    s += u_old[g.iu(i - 1, j + 1)];
                    cnt++;
                }
                if (cnt > 0)
                    uL = s / cnt;
            ...
            if (i < nx) {
                ...
                if (j < ny && !g.is_solid(i, j + 1) && !g.is_solid(i + 1, j + 1)) {
                    s += u_old[g.iu(i, j + 1)];
                    cnt++;
                }
                if (cnt > 0)
