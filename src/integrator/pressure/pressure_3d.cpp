...
void PressureProjection3D::project(Grid3D& g, double dt, Solver3D& solver, int max_iter,
                                   double tol) {
    ...
    for (int k = 1; k <= nz; k++)
        for (int j = 1; j <= ny; j++)
            for (int i = 1; i < nx; i++) {
                if (g.is_solid(i, j, k) || g.is_solid(i + 1, j, k))
                    continue;
    ...
    for (int k = 1; k <= nz; k++)
        for (int j = 1; j < ny; j++)
            for (int i = 1; i <= nx; i++) {
                if (g.is_solid(i, j, k) || g.is_solid(i, j + 1, k))
                    continue;
