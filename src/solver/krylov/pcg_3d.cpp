...
PCG3D::PCG3D(std::unique_ptr<Preconditioner3D> p) : precond_(std::move(p)) {}

std::string PCG3D::name() const {
    return "PCG3D(" + precond_->name() + ")";
}

void PCG3D::solve(Grid3D& g, const std::vector<double>& rhs_in, int max_iter, double tol) {
    ...
    std::vector<double> rhs = rhs_in;
    {
        ...
        for (int i = 1; i <= nx; i++)
            for (int j = 1; j <= ny; j++)
                for (int k = 1; k <= nz; k++)
                    if (!g.is_solid(i, j, k))
                        rhs[g.ip(i, j, k)] = -(rhs[g.ip(i, j, k)] - mean);
    }
        ...
        for (int i = 1; i <= nx; i++)
            for (int j = 1; j <= ny; j++)
                for (int k = 1; k <= nz; k++) {
                    ...
                    double vR =
                        (i < nx && !g.is_solid(i + 1, j, k)) ? v[g.ip(i + 1, j, k)] : v[idx];
                    double vB = (j > 1 && !g.is_solid(i, j - 1, k)) ? v[g.ip(i, j - 1, k)] : v[idx];
                    double vT =
                        (j < ny && !g.is_solid(i, j + 1, k)) ? v[g.ip(i, j + 1, k)] : v[idx];
                    double vF = (k > 1 && !g.is_solid(i, j, k - 1)) ? v[g.ip(i, j, k - 1)] : v[idx];
                    double vK =
                        (k < nz && !g.is_solid(i, j, k + 1)) ? v[g.ip(i, j, k + 1)] : v[idx];
                    Av[idx] =
                        diag * v[idx] - (vL + vR) * idx2 - (vB + vT) * idy2 - (vF + vK) * idz2;
                }
    };
