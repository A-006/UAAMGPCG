...
void Jacobi::solve(Grid& g, const std::vector<double>& rhs_in, int max_iter, double /*tol*/) {
    ...
    for (int iter = 0; iter < max_iter; iter++) {
        for (int i = 1; i <= nx; i++) {
            for (int j = 1; j <= ny; j++) {
                ...
                double pL =
                    (i > 1 && !g.is_solid(i - 1, j)) ? p_old[g.ip(i - 1, j)] : p_old[g.ip(i, j)];
                double pR =
                    (i < nx && !g.is_solid(i + 1, j)) ? p_old[g.ip(i + 1, j)] : p_old[g.ip(i, j)];
                double pB =
                    (j > 1 && !g.is_solid(i, j - 1)) ? p_old[g.ip(i, j - 1)] : p_old[g.ip(i, j)];
                double pT =
                    (j < ny && !g.is_solid(i, j + 1)) ? p_old[g.ip(i, j + 1)] : p_old[g.ip(i, j)];
