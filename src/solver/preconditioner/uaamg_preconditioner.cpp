...
void UAAMGPreconditioner::setupFineCoeffs(Level& L) {
    ...
    for (int i = 1; i <= nx; i++)
        for (int j = 1; j <= ny; j++) {
            ...
            if (L.solid[id])
                continue;
            if (i < nx && !L.solid[cidx(i + 1, j, s)])
                L.cx[id] = idx2;
            if (j < ny && !L.solid[cidx(i, j + 1, s)])
                L.cy[id] = idy2;
        }
    for (int i = 1; i <= nx; i++)
        for (int j = 1; j <= ny; j++) {
...
void UAAMGPreconditioner::galerkinCoarsen(const Level& fine, Level& coarse) {
    ...
    for (int ic = 1; ic <= coarse.nx; ic++)
        for (int jc = 1; jc <= coarse.ny; jc++) {
            ...
            double sx = 0, sy = 0;
            for (int dj = 0; dj < 2; dj++)
                sx += fine.cx[cidx(i_f + 1, j_f + dj, fs)]; // +x edge = fine i_f+1
            for (int di = 0; di < 2; di++)
                sy += fine.cy[cidx(i_f + di, j_f + 1, fs)];
            coarse.cx[cid] = sx;
            coarse.cy[cid] = sy;
        }
    for (int ic = 1; ic <= coarse.nx; ic++)
        for (int jc = 1; jc <= coarse.ny; jc++) {
            ...
            if (coarse.solid[cid])
                continue;
            coarse.diag[cid] = coarse.cx[cid] + coarse.cx[cidx(ic - 1, jc, cs)] + coarse.cy[cid] +
                               coarse.cy[cidx(ic, jc - 1, cs)];
        }
}
