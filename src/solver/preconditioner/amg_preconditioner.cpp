...
static inline int aidx(int i, int j, int stride) {
    return i + j * stride;
}
    ...
    for (int s = 0; s < sweeps; s++) {
        for (int i = 1; i <= nx; i++)
            for (int j = 1 + (i % 2); j <= ny; j += 2) {
                ...
                if (L.solid[id])
                    continue;
                double xL    = (i > 1 && !L.solid[aidx(i - 1, j, stride)])
                                   ? L.p[aidx(i - 1, j, stride)]
                                   : L.p[id];
                double xR    = (i < nx && !L.solid[aidx(i + 1, j, stride)])
                                   ? L.p[aidx(i + 1, j, stride)]
                                   : L.p[id];
                double xB    = (j > 1 && !L.solid[aidx(i, j - 1, stride)])
                                   ? L.p[aidx(i, j - 1, stride)]
                                   : L.p[id];
                double xT    = (j < ny && !L.solid[aidx(i, j + 1, stride)])
                                   ? L.p[aidx(i, j + 1, stride)]
                                   : L.p[id];
                double lap   = (xL + xR) * idx2 + (xB + xT) * idy2;
                double eff_d = diag_full;
                if (i == 1 || L.solid[aidx(i - 1, j, stride)])
                    eff_d -= idx2;
                if (i == nx || L.solid[aidx(i + 1, j, stride)])
                    eff_d -= idx2;
                if (j == 1 || L.solid[aidx(i, j - 1, stride)])
                    eff_d -= idy2;
                if (j == ny || L.solid[aidx(i, j + 1, stride)])
                    eff_d -= idy2;
                L.p[id] +=
                    ((eff_d < 1e-15) ? 0.0 : 1.0 / eff_d) * (L.b[id] - diag_full * L.p[id] + lap);
            }
        for (int i = 1; i <= nx; i++)
...
void AMGPreconditioner::restrictResidual(const AggLevel& fine, AggLevel& coarse) {
    ...
    for (int i = 1; i <= fnx; i++)
        for (int j = 1; j <= fny; j++) {
            ...
            if (fine.solid[idx])
                continue;
            double xL  = (i > 1 && !fine.solid[aidx(i - 1, j, fs)]) ? fine.p[aidx(i - 1, j, fs)]
                                                                    : fine.p[idx];
            double xR  = (i < fnx && !fine.solid[aidx(i + 1, j, fs)]) ? fine.p[aidx(i + 1, j, fs)]
                                                                      : fine.p[idx];
            double xB  = (j > 1 && !fine.solid[aidx(i, j - 1, fs)]) ? fine.p[aidx(i, j - 1, fs)]
                                                                    : fine.p[idx];
            double xT  = (j < fny && !fine.solid[aidx(i, j + 1, fs)]) ? fine.p[aidx(i, j + 1, fs)]
                                                                      : fine.p[idx];
...
void AMGPreconditioner::apply(const Grid& g, const std::vector<double>& r, std::vector<double>& z) {
