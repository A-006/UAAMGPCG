    ...
    for (int i = 1; i <= nx; i++)
        for (int j = 1; j <= ny; j++)
            for (int k = 1; k <= nz; k++) {
                ...
                if (L.solid[id])
                    continue;
                if (i < nx && !L.solid[cidx(i + 1, j, k, nx, ny)])
                    L.cx[id] = idx2;
                if (j < ny && !L.solid[cidx(i, j + 1, k, nx, ny)])
                    L.cy[id] = idy2;
                if (k < nz && !L.solid[cidx(i, j, k + 1, nx, ny)])
                    L.cz[id] = idz2;
            }
    for (int i = 1; i <= nx; i++)
        for (int j = 1; j <= ny; j++)
            for (int k = 1; k <= nz; k++) {
                ...
                if (L.solid[id])
                    continue;
                L.diag[id] = L.cx[id] + L.cx[cidx(i - 1, j, k, nx, ny)] + L.cy[id] +
                             L.cy[cidx(i, j - 1, k, nx, ny)] + L.cz[id] +
                             L.cz[cidx(i, j, k - 1, nx, ny)];
            }
}
            ...
            for (int j = 1; j <= ny; j++)
                for (int k = 1; k <= nz; k++) {
                    if (((i + j + k) & 1) != parity)
                        continue;
                    int id = cidx(i, j, k, nx, ny);
                    if (L.solid[id] || L.diag[id] < 1e-30)
                        continue;
                    int idxm = cidx(i - 1, j, k, nx, ny), idym = cidx(i, j - 1, k, nx, ny),
                        idzm  = cidx(i, j, k - 1, nx, ny);
                    ...
                    for (int dj = 0; dj < 2; dj++)
                        for (int dk = 0; dk < 2; dk++) {
                            int fi = i_f + di, fj = j_f + dj, fk = k_f + dk,
                                fidx = cidx(fi, fj, fk, fnx, fny);
                            if (fine.solid[fidx])
                                continue;
                            int im    = cidx(fi - 1, fj, fk, fnx, fny),
                                jm    = cidx(fi, fj - 1, fk, fnx, fny),
                                km    = cidx(fi, fj, fk - 1, fnx, fny);
                            double Ax = fine.diag[fidx] * fine.p[fidx] -
                                        fine.cx[fidx] * fine.p[cidx(fi + 1, fj, fk, fnx, fny)] -
                                        fine.cx[im] * fine.p[im] -
                                        fine.cy[fidx] * fine.p[cidx(fi, fj + 1, fk, fnx, fny)] -
                                        fine.cy[jm] * fine.p[jm] -
                                        fine.cz[fidx] * fine.p[cidx(fi, fj, fk + 1, fnx, fny)] -
                                        fine.cz[km] * fine.p[km];
                            sum += fine.b[fidx] - Ax;
                        }
                coarse.b[cid] = sum;
            }
...
void UAAMGPreconditioner3D::apply(const Grid3D& g, const std::vector<double>& r,
    ...
    for (int l = 1; l < nl; l++)
        galerkinCoarsen(levels_[l - 1], levels_[l]);
