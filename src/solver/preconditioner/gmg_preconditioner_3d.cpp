        ...
        for (int i = 1; i <= L.nx; i++)
            for (int j = 1; j <= L.ny; j++)
                for (int k = 1; k <= L.nz; k++) {
                    ...
                    double eff_d = diag;
                    if (i == 1 || L.solid[cidx(i - 1, j, k, L.nx, L.ny)])
                        eff_d -= idx2;
                    if (i == L.nx || L.solid[cidx(i + 1, j, k, L.nx, L.ny)])
                        eff_d -= idx2;
                    if (j == 1 || L.solid[cidx(i, j - 1, k, L.nx, L.ny)])
                        eff_d -= idy2;
                    if (j == L.ny || L.solid[cidx(i, j + 1, k, L.nx, L.ny)])
                        eff_d -= idy2;
                    if (k == 1 || L.solid[cidx(i, j, k - 1, L.nx, L.ny)])
                        eff_d -= idz2;
                    if (k == L.nz || L.solid[cidx(i, j, k + 1, L.nx, L.ny)])
                        eff_d -= idz2;

                    L.p[id] +=
                        ((eff_d < 1e-15) ? 0.0 : 1.0 / eff_d) * (L.b[id] - diag * L.p[id] + lap);
                }
                ...
                for (int di = 0; di < 2; di++)
                    for (int dj = 0; dj < 2; dj++)
                        for (int dk = 0; dk < 2; dk++) {
                            ...
                            double pL =
                                (fi > 1 && !fine.solid[cidx(fi - 1, fj, fk, fine.nx, fine.ny)])
                                    ? fine.p[cidx(fi - 1, fj, fk, fine.nx, fine.ny)]
                                    : fine.p[fidx];
                            double pR = (fi < fine.nx &&
                                         !fine.solid[cidx(fi + 1, fj, fk, fine.nx, fine.ny)])
                                            ? fine.p[cidx(fi + 1, fj, fk, fine.nx, fine.ny)]
                                            : fine.p[fidx];
                            double pB =
                                (fj > 1 && !fine.solid[cidx(fi, fj - 1, fk, fine.nx, fine.ny)])
                                    ? fine.p[cidx(fi, fj - 1, fk, fine.nx, fine.ny)]
                                    : fine.p[fidx];
                            double pT = (fj < fine.ny &&
                                         !fine.solid[cidx(fi, fj + 1, fk, fine.nx, fine.ny)])
                                            ? fine.p[cidx(fi, fj + 1, fk, fine.nx, fine.ny)]
                                            : fine.p[fidx];
                            double pF =
                                (fk > 1 && !fine.solid[cidx(fi, fj, fk - 1, fine.nx, fine.ny)])
                                    ? fine.p[cidx(fi, fj, fk - 1, fine.nx, fine.ny)]
                                    : fine.p[fidx];
                            double pK = (fk < fine.nz &&
                                         !fine.solid[cidx(fi, fj, fk + 1, fine.nx, fine.ny)])
                                            ? fine.p[cidx(fi, fj, fk + 1, fine.nx, fine.ny)]
                                            : fine.p[fidx];
