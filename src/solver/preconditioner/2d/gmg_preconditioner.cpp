 ...
 * @brief Geometric Multigrid V-cycle — symmetric V-cycle, column-major indexing.
 * @author liutao
 * @date 2026-05-22
 */
...
static inline int cidx(int i, int j, int stride) {
    return i + j * stride;
}
    ...
    for (int s = 0; s < sweeps; s++) {
        for (int i = 1; i <= L.nx; i++)
            for (int j = 1 + (i % 2); j <= L.ny; j += 2) {
                ...
                if (L.solid[id])
                    continue;
                double pL    = (i > 1 && !L.solid[cidx(i - 1, j, stride)])
                                   ? L.p[cidx(i - 1, j, stride)]
                                   : L.p[id];
                double pR    = (i < L.nx && !L.solid[cidx(i + 1, j, stride)])
                                   ? L.p[cidx(i + 1, j, stride)]
                                   : L.p[id];
                double pB    = (j > 1 && !L.solid[cidx(i, j - 1, stride)])
                                   ? L.p[cidx(i, j - 1, stride)]
                                   : L.p[id];
                double pT    = (j < L.ny && !L.solid[cidx(i, j + 1, stride)])
                                   ? L.p[cidx(i, j + 1, stride)]
                                   : L.p[id];
                double lap   = (pL + pR) * idx2 + (pB + pT) * idy2;
                double eff_d = diag;
                if (i == 1 || L.solid[cidx(i - 1, j, stride)])
                    eff_d -= idx2;
                if (i == L.nx || L.solid[cidx(i + 1, j, stride)])
                    eff_d -= idx2;
                if (j == 1 || L.solid[cidx(i, j - 1, stride)])
                    eff_d -= idy2;
                if (j == L.ny || L.solid[cidx(i, j + 1, stride)])
                    eff_d -= idy2;
...
void GMGPreconditioner::smoothReverse(Level& L, int sweeps) {
    double idx2 = 1.0 / (L.dx * L.dx), idy2 = 1.0 / (L.dy * L.dy);
    double diag = 2.0 * (idx2 + idy2);
    int stride  = L.nx + 2;
    for (int s = 0; s < sweeps; s++) {
        ...
        for (int jc = 1; jc <= coarse.ny; jc++) {
            ...
            for (int di = 0; di < 2; di++)
                for (int dj = 0; dj < 2; dj++) {
                    ...
                    if (fine.solid[fidx])
                        continue;
                    double pL  = (fi > 1 && !fine.solid[cidx(fi - 1, fj, fs)])
                                     ? fine.p[cidx(fi - 1, fj, fs)]
                                     : fine.p[fidx];
                    double pR  = (fi < fine.nx && !fine.solid[cidx(fi + 1, fj, fs)])
                                     ? fine.p[cidx(fi + 1, fj, fs)]
                                     : fine.p[fidx];
                    double pB  = (fj > 1 && !fine.solid[cidx(fi, fj - 1, fs)])
                                     ? fine.p[cidx(fi, fj - 1, fs)]
                                     : fine.p[fidx];
                    double pT  = (fj < fine.ny && !fine.solid[cidx(fi, fj + 1, fs)])
                                     ? fine.p[cidx(fi, fj + 1, fs)]
                                     : fine.p[fidx];
                    double lap = (pL + pR) * idx2 + (pB + pT) * idy2;
...
void GMGPreconditioner::apply(const Grid& g, const std::vector<double>& r, std::vector<double>& z) {
