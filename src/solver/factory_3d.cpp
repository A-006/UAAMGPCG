...
std::unique_ptr<Solver3D> Factory3D::create(const std::string& name) {
    ...
    if (name == "rbgs")
        return std::make_unique<RBGS3D>();
    if (name == "cg")
        return std::make_unique<PCG3D>(std::make_unique<IdentityPreconditioner3D>());
    // if (name == "pcg")       return
    // std::make_unique<PCG3D>(std::make_unique<GMGPreconditioner3D>()); if (name == "pcg_gmg")
    // return std::make_unique<PCG3D>(std::make_unique<GMGPreconditioner3D>()); if (name ==
    // "pcg_amg")   return std::make_unique<PCG3D>(std::make_unique<AMGPreconditioner3D>()); if
    // (name == "pcg_uaamg") return
    // std::make_unique<PCG3D>(std::make_unique<UAAMGPreconditioner3D>()); Fallback: return Jacobi3D
    // for pcg/pcg_gmg/pcg_amg/pcg_uaamg until 3D preconditioners exist
