...
__device__ inline int didx(int i, int j, int s) {
    return i + j * s;
}

__global__ void cg_matvec(const double* p, double* Ap, const bool* solid, int nx, int ny,
...
__global__ void cg_axpy(double* y, const double* x, double a, const bool* solid, int nx, int ny,
                        int stride) {
    int i = blockIdx.x * blockDim.x + threadIdx.x + 1;
    int j = blockIdx.y * blockDim.y + threadIdx.y + 1;
    if (i > nx || j > ny)
...
__global__ void cg_submean_2d(double* v, double m, const bool* solid, int nx, int ny, int stride) {
    int i = blockIdx.x * blockDim.x + threadIdx.x + 1;
    int j = blockIdx.y * blockDim.y + threadIdx.y + 1;
    if (i > nx || j > ny)
        return;
...
__global__ void cg_negate_2d(double* v, const bool* solid, int nx, int ny, int stride) {
    int i = blockIdx.x * blockDim.x + threadIdx.x + 1;
    int j = blockIdx.y * blockDim.y + threadIdx.y + 1;
    if (i > nx || j > ny)
        return;
...
__global__ void cg_sum_2d(const double* v, const bool* solid, int nx, int ny, int stride,
                          double* part) {
...
__global__ void cg_dot_partial(const double* a, const double* b, const bool* solid, int N,
                               double* part) {
    __shared__ double s[256];
    int tid    = threadIdx.x;
    double sum = 0;
...
static double gpu_mean(const double* v, const bool* solid, int nx, int ny, int stride, double* part,
                       int* cnt, int nb) {
    cg_sum_2d<<<nb, 256>>>(v, solid, nx, ny, stride, part);
    cudaDeviceSynchronize();
    double s = host_sum(part, nb);
    cg_count_2d<<<nb, 256>>>(solid, nx, ny, stride, cnt);
    cudaDeviceSynchronize();
