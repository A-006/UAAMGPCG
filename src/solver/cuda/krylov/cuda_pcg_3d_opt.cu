...
template <int BS> __device__ double block_reduce(double val, double* smem, int tid) {
    smem[tid] = val;
    __syncthreads();
...
__global__ void matvec_tiled_dot_kernel(const double* p, double* Ap, const bool* solid, int nx,
                                        int ny, int nz, int pitch, double idx2, double idy2,
                                        double idz2, double diag, double* dot_buf) {
...
// ═══════════════════════════════════════════════════════════════
__global__ void axpy_dot_kernel(double* y, const double* x, double a, const bool* solid, int nx,
                                int ny, int nz, int pitch, double* dot_buf) {
...
__global__ void dot_kernel_1d(const double* a, const double* b, const bool* solid, int N,
                              double* part) {
...
__global__ void sum_interior_kernel(const double* v, const bool* solid, int nx, int ny, int nz,
...
__global__ void count_interior_kernel(const bool* solid, int nx, int ny, int nz, int pitch,
                                      int* part) {
    ...
    for (int lin = blockIdx.x * blockDim.x + tid; lin < nx * ny * nz;
         lin += blockDim.x * gridDim.x) {
        int i = (lin % nx) + 1, j = ((lin / nx) % ny) + 1, k = (lin / (nx * ny)) + 1;
        if (!solid[opti_idx(i, j, k, pitch, ny)])
            sum++;
    }
    s[tid] = sum;
    __syncthreads();
...
__global__ void submean_kernel(double* v, double m, const bool* solid, int nx, int ny, int nz,
                               int pitch) {
    int i = blockIdx.x * blockDim.x + threadIdx.x + 1,
...
__global__ void axpy_kernel(double* y, const double* x, double a, const bool* solid, int nx, int ny,
                            int nz, int pitch) {
    int i = blockIdx.x * blockDim.x + threadIdx.x + 1,
        j = blockIdx.y * blockDim.y + threadIdx.y + 1,
        k = blockIdx.z * blockDim.z + threadIdx.z + 1;
...
static double gpu_sum(const double* d_part, int n, double* d_result) {
    ...
    double val;
    cudaMemcpy(&val, d_result, sizeof(double), cudaMemcpyDeviceToHost);
    return val;
}
...
static double gpu_mean(const double* v, const bool* solid, int nx, int ny, int nz, int pitch,
                       double* part, int* cnt, int nb) {
    sum_interior_kernel<<<nb, 256>>>(v, solid, nx, ny, nz, pitch, part);
    cudaDeviceSynchronize();
    double s = gpu_sum(part, nb, part);
    count_interior_kernel<<<nb, 256>>>(solid, nx, ny, nz, pitch, cnt);
    cudaDeviceSynchronize();
...
__global__ void axpy_dev_k(double* y, const double* x, const double* a, const bool* s, int nx,
...
__global__ void submean_dev_k(double* v, const double* m, const bool* s, int nx, int ny, int nz,
                              int pitch) {
    int i = blockIdx.x * blockDim.x + threadIdx.x + 1,
        j = blockIdx.y * blockDim.y + threadIdx.y + 1,
        k = blockIdx.z * blockDim.z + threadIdx.z + 1;
...
__global__ void pupd_dev_k(double* p, const double* z, const double* b, const bool* s, int nx,
                           int ny, int nz, int pitch) { // p = z + beta*p  (search-direction update)
    int i = blockIdx.x * blockDim.x + threadIdx.x + 1,
        j = blockIdx.y * blockDim.y + threadIdx.y + 1,
        k = blockIdx.z * blockDim.z + threadIdx.z + 1;
    if (i > nx || j > ny || k > nz)
...
__global__ void sc_div_k(double* o, const double* n, const double* d) {
    *o = *n / *d;
}
__global__ void sc_negdiv_k(double* o, const double* n, const double* d) {
    *o = -(*n) / *d;
}
__global__ void sc_meandiv_k(double* o, const double* s, double cnt) {
    *o = (cnt > 0) ? *s / cnt : 0.0;
}
__global__ void sc_copy_k(double* o, const double* i) {
    *o = *i;
}

// ── Mixed precision: FP64↔FP32 casts + FP32 preconditioner apply ──
__global__ void cast_d2f_k(const double* d, float* f, int N) {
    int i = blockIdx.x * 256 + threadIdx.x;
    if (i < N)
        f[i] = (float)d[i];
}
__global__ void cast_f2d_k(const float* f, double* d, int N) {
    int i = blockIdx.x * 256 + threadIdx.x;
    if (i < N)
        d[i] = (double)f[i];
}
...
void CudaPCG3D::solve_optimized(CudaGrid3D& g, double* p, double* rhs, int max_iter, double tol) {
    ...
    if (mixed_) { // FP32 preconditioner: float grid + setup once
        if (gf_N_ < N) {
            ...
            cudaMalloc(&d_rf, N * sizeof(float));
            cudaMalloc(&d_zf, N * sizeof(float));
            gf_N_ = N;
        }
    ...
    if (dbg)
        std::printf("[PCG] r0_sq=%.3e tol_abs_sq=%.3e\n", r0_sq, tol_abs_sq);
    for (int k = 0; k < max_iter; k++) {
        // ── FUSED: matvec(tiled) + dot(p,Ap) ──
        matvec_tiled_dot_kernel<<<grid3d, block3d>>>(d_p, d_Ap, g.solid, nx, ny, nz, pitch, g.idx2,
                                                     g.idy2, g.idz2, g.diag, d_dot_buf);
        cudaDeviceSynchronize();
        ...
        axpy_dot_kernel<<<grid3d, block3d>>>(d_r, d_Ap, -alpha, g.solid, nx, ny, nz, pitch,
                                             d_dot_buf);
        cudaDeviceSynchronize();
        ...
        if (dbg && (k < 5 || k % 5 == 0))
            std::printf("[PCG]  k=%3d rsnew=%.3e rel=%.3e\n", k, rsnew,
                        std::sqrt(rsnew / std::max(r0_sq, 1e-30)));
        if (rsnew < tol_abs_sq) {
            last_iters   = k + 1;
...
void CudaPCG3D::solve_device(CudaGrid3D& g, double* p, double* rhs, int iters, double /*tol*/) {
    ...
    double* d;
    cudaMalloc(&d,
               8 * sizeof(double)); // 0 rsold,1 pAp,2 rsnew,3 rz,4 alpha,5 negalpha,6 beta,7 mean
    ...
    cudaMemset(p, 0, N * sizeof(double));
    dmean(d_r);
    negate_kernel<<<grd, blk>>>(d_r, g.solid, nx, ny, nz, pitch); // r = -(rhs-mean)
    precond_->vcycle_apply(g, d_r, d_z);
    dmean(d_z);
    cudaMemcpy(d_p, d_z, N * sizeof(double), cudaMemcpyDeviceToDevice);
    dot_kernel_1d<<<nb1d, 256>>>(d_r, d_z, g.solid, N, d_dot_buf);
    ...
    for (int k = 0; k < iters; k++) {
        matvec_tiled_dot_kernel<<<grd, blk>>>(d_p, d_Ap, g.solid, nx, ny, nz, pitch, g.idx2, g.idy2,
                                              g.idz2, g.diag, d_dot_buf);
