...
__device__ inline int idx2d(int i, int j, int s) {
    return i + j * s;
}
...
__global__ void sum_interior_kernel(const double* v, const bool* solid, int nx, int ny, int stride,
                                    double* part) {
...
__global__ void count_interior_kernel(const bool* solid, int nx, int ny, int stride, int* part) {
    __shared__ int s[256];
    int tid = threadIdx.x;
    int sum = 0;
    for (int k = blockIdx.x * blockDim.x + tid; k < nx * ny; k += blockDim.x * gridDim.x) {
        int i = (k % nx) + 1, j = (k / nx) + 1;
        if (!solid[idx2d(i, j, stride)])
            sum++;
    }
    s[tid] = sum;
    __syncthreads();
...
__global__ void subtract_mean_kernel(double* v, double mean, const bool* solid, int nx, int ny,
                                     int stride) {
...
__global__ void negate_kernel(double* v, const bool* solid, int nx, int ny, int stride) {
    int i = blockIdx.x * blockDim.x + threadIdx.x + 1;
    int j = blockIdx.y * blockDim.y + threadIdx.y + 1;
    if (i > nx || j > ny)
        return;
...
static double compute_mean(const double* d_v, const bool* d_solid, int nx, int ny, int stride,
...
void CudaPCG::solve(CudaGrid& g, double* p, double* rhs, int max_iter, double tol) {
    ...
    for (int k = 0; k < max_iter; k++) {
        matvec_kernel<<<grid2d, block2d>>>(d_p, d_Ap, g.solid, nx, ny, stride, g.idx2, g.idy2,
                                           g.diag);
