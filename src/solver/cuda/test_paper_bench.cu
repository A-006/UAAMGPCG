...
int main() {
    ...
    for (auto& B : grids) {
        ...
        //    UAAMG_NOTRIM=1 to A/B the §5.4 trimming speedup. ──
        {
            ...
            for (int w = 0; w < 5; w++)
                precond.vcycle_apply(g, d_rhs, d_z);
            cudaDeviceSynchronize();
        ...
        double gpu_ms = 0, gpu_l2 = 0, cpu_ms = 0, cpu_l2 = 0;
        {
            ...
            cudaMemcpy(gr.data(), d_rhs, N * sizeof(double), cudaMemcpyDeviceToHost);
            std::vector<char> gs(N);
            cudaMemcpy(gs.data(), g.solid, N * sizeof(bool), cudaMemcpyDeviceToHost);
            gpu_l2 = residual_l2(gp.data(), gr.data(), (bool*)gs.data(), nx, ny, nz, 1.0 / nx,
                                 1.0 / ny, 1.0 / nz);
        }
        ...
        if (ncells <= 5000000) { // include 256x128x128 for CPU vs GPU speedup
            ...
            for (int i = 1; i <= nx; i++)
                for (int j = 1; j <= ny; j++)
                    for (int k = 1; k <= nz; k++)
                        if (!cg.is_solid(i, j, k)) {
                            sum += h_rhs[cg.ip(i, j, k)];
                            cnt++;
                        }
            double mean = cnt > 0 ? sum / cnt : 0;
            double sr   = 0;
            for (int i = 1; i <= nx; i++)
                for (int j = 1; j <= ny; j++)
                    for (int k = 1; k <= nz; k++) {
                        ...
                        double pC = cg.p[id];
                        double pL =
                            (i > 1 && !cg.is_solid(i - 1, j, k)) ? cg.p[cg.ip(i - 1, j, k)] : pC;
                        double pR =
                            (i < nx && !cg.is_solid(i + 1, j, k)) ? cg.p[cg.ip(i + 1, j, k)] : pC;
                        double pB =
                            (j > 1 && !cg.is_solid(i, j - 1, k)) ? cg.p[cg.ip(i, j - 1, k)] : pC;
                        double pT =
                            (j < ny && !cg.is_solid(i, j + 1, k)) ? cg.p[cg.ip(i, j + 1, k)] : pC;
                        double pF =
                            (k > 1 && !cg.is_solid(i, j, k - 1)) ? cg.p[cg.ip(i, j, k - 1)] : pC;
                        double pK =
                            (k < nz && !cg.is_solid(i, j, k + 1)) ? cg.p[cg.ip(i, j, k + 1)] : pC;
                        double idx2 = 1.0 / (cg.dx * cg.dx), idy2 = 1.0 / (cg.dy * cg.dy),
                               idz2 = 1.0 / (cg.dz * cg.dz);
                        double diag = 2.0 * (idx2 + idy2 + idz2);
                        double Ax =
                            diag * pC - (pL + pR) * idx2 - (pB + pT) * idy2 - (pF + pK) * idz2;
                        double r = -(h_rhs[id] - mean) - Ax;
                        sr += r * r;
                    }
            cpu_l2 = std::sqrt(sr);
        }
        ...
        double pcg_opt_rel = 1.0;
        {
            ...
            for (int w = 0; w < 2; w++) {
                cudaMemset(d_p, 0, N * sizeof(double));
                pcg_opt.solve_optimized(g, d_p, d_rhs, 5, 1e-6);
            }
            cudaDeviceSynchronize();
            cudaMemset(d_p, 0, N * sizeof(double));
            cudaMemcpy(d_rhs, h_rhs.data(), N * sizeof(double), cudaMemcpyHostToDevice);
            ...
            std::vector<char> gs(N);
            cudaMemcpy(gs.data(), g.solid, N * sizeof(bool), cudaMemcpyDeviceToHost);
            gpu_opt_l2 = residual_l2(gp.data(), gr.data(), (bool*)gs.data(), nx, ny, nz, 1.0 / nx,
                                     1.0 / ny, 1.0 / nz);
        }
        ...
        double mix_rel    = 1.0;
        {
            ...
            for (int w = 0; w < 2; w++) {
                cudaMemset(d_p, 0, N * sizeof(double));
                pcg_mix.solve_mixed(g, d_p, d_rhs, 5, 1e-6);
            }
            cudaDeviceSynchronize();
            cudaMemset(d_p, 0, N * sizeof(double));
            cudaMemcpy(d_rhs, h_rhs.data(), N * sizeof(double), cudaMemcpyHostToDevice);
            ...
            mix_iters  = pcg_mix.last_iters;
            mix_rel    = pcg_mix.last_rel_res;
        }

        // ── Report ──
        printf("  GPU PCG(%d):     %8.2f ms  L2|res|=%.4e\n", B.pcg_iters, gpu_ms, gpu_l2);
        printf("  GPU PCG-opt:    %8.2f ms  iters=%d  rel_res=%.2e  L2|res|=%.4e\n", gpu_opt_ms,
               pcg_opt_iters, pcg_opt_rel, gpu_opt_l2);
        printf("  GPU PCG-mixed:  %8.2f ms  iters=%d  rel_res=%.2e   (FP64 CG + FP32 precond, "
               "%.2fx vs opt)\n",
               gpu_mix_ms, mix_iters, mix_rel, gpu_opt_ms > 0 ? gpu_opt_ms / gpu_mix_ms : 0.0);
        if (cpu_ms > 0)
            printf("  CPU PCG(%d):     %8.2f ms  L2|res|=%.4e  speedup=%.1fx\n", B.pcg_iters,
                   cpu_ms, cpu_l2, gpu_ms > 0 ? cpu_ms / gpu_ms : 0);
    ...
    for (int i = 1; i <= nx; i++)
        for (int j = 1; j <= ny; j++)
            for (int k = 1; k <= nz; k++)
                hr[i + j * (nx + 2) + k * (nx + 2) * (ny + 2)] =
                    (double)(i + j + k) / (nx + ny + nz);
