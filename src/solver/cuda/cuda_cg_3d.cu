...
__global__ void cg3d_matvec(const double* p, double* Ap, const bool* solid, int nx, int ny, int nz,
                            int pitch, double idx2, double idy2, double idz2, double diag) {
...
__global__ void cg3d_axpy(double* y, const double* x, double a, const bool* solid, int nx, int ny,
                          int nz, int pitch) {
    int i = blockIdx.x * blockDim.x + threadIdx.x + 1;
    int j = blockIdx.y * blockDim.y + threadIdx.y + 1;
    int k = blockIdx.z * blockDim.z + threadIdx.z + 1;
...
__global__ void cg3d_submean_3d(double* v, double m, const bool* solid, int nx, int ny, int nz,
                                int pitch) {
    int i = blockIdx.x * blockDim.x + threadIdx.x + 1;
    int j = blockIdx.y * blockDim.y + threadIdx.y + 1;
    int k = blockIdx.z * blockDim.z + threadIdx.z + 1;
...
__global__ void cg3d_negate_3d(double* v, const bool* solid, int nx, int ny, int nz, int pitch) {
    int i = blockIdx.x * blockDim.x + threadIdx.x + 1;
    int j = blockIdx.y * blockDim.y + threadIdx.y + 1;
    int k = blockIdx.z * blockDim.z + threadIdx.z + 1;
    if (i > nx || j > ny || k > nz)
...
__global__ void cg3d_sum_3d(const double* v, const bool* solid, int nx, int ny, int nz, int pitch,
                            double* part) {
...
__global__ void cg3d_count_3d(const bool* solid, int nx, int ny, int nz, int pitch, int* part) {
...
__global__ void cg3d_dot_partial(const double* a, const double* b, const bool* solid, int N,
                                 double* part) {
    __shared__ double s[256];
    int tid    = threadIdx.x;
    double sum = 0;
...
static double gpu_mean_3d(const double* v, const bool* solid, int nx, int ny, int nz, int pitch,
                          double* part, int* cnt, int nb) {
    cg3d_sum_3d<<<nb, 256>>>(v, solid, nx, ny, nz, pitch, part);
    cudaDeviceSynchronize();
    double s = host_sum(part, nb);
    cg3d_count_3d<<<nb, 256>>>(solid, nx, ny, nz, pitch, cnt);
    cudaDeviceSynchronize();
...
void CudaCG3D::solve(CudaGrid3D& g, double* p, double* rhs, int max_iter, double tol) {
    ...
    for (int k = 0; k < max_iter; k++) {
        cg3d_matvec<<<grid3d, block3d>>>(d_p, d_Ap, g.solid, nx, ny, nz, pitch, g.idx2, g.idy2,
                                         g.idz2, g.diag);
        cudaDeviceSynchronize();
