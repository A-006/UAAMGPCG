...
__device__ inline int fidx(int i, int j, int k, int pitch, int ny) {
...
__global__ __launch_bounds__(512, 2) void matvec_tiled_dot_kernel_f(
    const float* __restrict p, float* __restrict Ap, const bool* __restrict solid, int nx, int ny,
    int nz, int pitch, float idx2, float idy2, float idz2, float diag, float* __restrict dot_buf) {
    ...
    if (valid) {
        int gid        = fidx(gi, gj, gk, pitch, ny);
        sp[li][lj][lk] = p[gid];
    ...
    if (tx == FT - 1) {
        ...
        if (gr <= nx && gj <= ny && gk <= nz) {
            int h              = fidx(gr, gj, gk, pitch, ny);
            sp[FT + 1][lj][lk] = p[h];
            ss[FT + 1][lj][lk] = solid[h];
        } else {
            ss[FT + 1][lj][lk] = true;
            sp[FT + 1][lj][lk] = 0.0f;
        }
    ...
    if (ty == FT - 1) {
        ...
        if (gi <= nx && gt <= ny && gk <= nz) {
            int h              = fidx(gi, gt, gk, pitch, ny);
            sp[li][FT + 1][lk] = p[h];
            ss[li][FT + 1][lk] = solid[h];
        } else {
            ss[li][FT + 1][lk] = true;
            sp[li][FT + 1][lk] = 0.0f;
        }
    ...
    if (tz == FT - 1) {
        ...
        if (gi <= nx && gj <= ny && gk2 <= nz) {
            int h              = fidx(gi, gj, gk2, pitch, ny);
            sp[li][lj][FT + 1] = p[h];
            ss[li][lj][FT + 1] = solid[h];
        } else {
            ss[li][lj][FT + 1] = true;
            sp[li][lj][FT + 1] = 0.0f;
        }
    ...
    if (valid && !ss[li][lj][lk]) {
        ...
        float pL = ss[li - 1][lj][lk] ? pC : sp[li - 1][lj][lk],
              pR = ss[li + 1][lj][lk] ? pC : sp[li + 1][lj][lk];
        float pB = ss[li][lj - 1][lk] ? pC : sp[li][lj - 1][lk],
              pT = ss[li][lj + 1][lk] ? pC : sp[li][lj + 1][lk];
        float pF = ss[li][lj][lk - 1] ? pC : sp[li][lj][lk - 1],
              pK = ss[li][lj][lk + 1] ? pC : sp[li][lj][lk + 1];
        Ap_val   = diag * pC - (pL + pR) * idx2 - (pB + pT) * idy2 - (pF + pK) * idz2;
        int gid  = fidx(gi, gj, gk, pitch, ny);
        Ap[gid]  = Ap_val;
        my_dot   = pC * Ap_val;
    }
    int tid   = tx + ty * FT + tz * (FT * FT);
    sdot[tid] = my_dot;
    __syncthreads();
    for (int s = (FT * FT * FT) / 2; s > 0; s >>= 1) {
        if (tid < s)
            sdot[tid] += sdot[tid + s];
        __syncthreads();
    }
...
__global__ __launch_bounds__(512, 2) void axpy_dot_kernel_f(float* __restrict y,
                                                            const float* __restrict x, float a,
                                                            const bool* __restrict solid, int nx,
                                                            int ny, int nz, int pitch,
                                                            float* __restrict dot_buf) {
    __shared__ float sdot[FT * FT * FT];
    ...
    float my_dot = 0.0f;
    if (i <= nx && j <= ny && k <= nz) {
        int id = fidx(i, j, k, pitch, ny);
        if (!solid[id]) {
            y[id] += a * x[id];
            my_dot = y[id] * y[id];
        }
    }
    int tid   = threadIdx.x + threadIdx.y * FT + threadIdx.z * (FT * FT);
    sdot[tid] = my_dot;
    __syncthreads();
    for (int s = (FT * FT * FT) / 2; s > 0; s >>= 1) {
        if (tid < s)
...
__global__ void dot_kernel_f(const float* a, const float* b, const bool* solid, int N,
                             float* part) {
...
__global__ void axpy_kernel_f(float* y, const float* x, float a, const bool* solid, int nx, int ny,
                              int nz, int pitch) {
    int i = blockIdx.x * blockDim.x + threadIdx.x + 1,
        j = blockIdx.y * blockDim.y + threadIdx.y + 1,
        k = blockIdx.z * blockDim.z + threadIdx.z + 1;
...
__global__ void submean_kernel_f(float* v, float m, const bool* solid, int nx, int ny, int nz,
                                 int pitch) {
    int i = blockIdx.x * blockDim.x + threadIdx.x + 1,
        j = blockIdx.y * blockDim.y + threadIdx.y + 1,
        k = blockIdx.z * blockDim.z + threadIdx.z + 1;
...
__global__ void sum_interior_kernel_f(const float* v, const bool* solid, int nx, int ny, int nz,
                                      int pitch, float* part) {
    __shared__ float s[256];
    int tid   = threadIdx.x;
    float sum = 0;
    for (int lin = blockIdx.x * blockDim.x + tid; lin < nx * ny * nz;
         lin += blockDim.x * gridDim.x) {
...
__global__ void count_interior_kernel_f(const bool* solid, int nx, int ny, int nz, int pitch,
                                        int* part) {
...
static float gpu_sum_f(const float* d_part, int n, float* d_result) {
    ...
    float val;
    cudaMemcpy(&val, d_result, sizeof(float), cudaMemcpyDeviceToHost);
    return val;
}

static float host_sum_f(const float* d, int n) {
...
static int host_sum_int_f(const int* d, int n) {
    std::vector<int> h(n);
...
static float gpu_mean_f(const float* v, const bool* solid, int nx, int ny, int nz, int pitch,
                        float* part, int* cnt, int nb) {
    sum_interior_kernel_f<<<nb, 256>>>(v, solid, nx, ny, nz, pitch, part);
    cudaDeviceSynchronize();
    float s = gpu_sum_f(part, nb, part);
    count_interior_kernel_f<<<nb, 256>>>(solid, nx, ny, nz, pitch, cnt);
    cudaDeviceSynchronize();
    int c = host_sum_int_f(cnt, nb);
    return c > 0 ? s / c : 0.0f;
}
...
struct CudaPCG3Df {
    ...
    void ensure(int N) {
        ...
        int mb = (N + 255) / 256 + 1;
        cudaMalloc(&d_dot, mb * sizeof(float));
        cudaMalloc(&d_cnt, mb * sizeof(int));
...
float CudaPCG3Df::solve_optimized(CudaGrid3Df& g, float* p, float* rhs, int max_iter, float tol) {
    ...
    dim3 block3d(FT, FT, FT), grid3d((nx + FT - 1) / FT, (ny + FT - 1) / FT, (nz + FT - 1) / FT);
    ...
    dot_kernel_f<<<nb1d, 256>>>(d_r, d_z, g.solid, N, d_dot);
    cudaDeviceSynchronize();
    ...
    for (int k = 0; k < max_iter; k++) {
        matvec_tiled_dot_kernel_f<<<grid3d, block3d>>>(d_p, d_Ap, g.solid, nx, ny, nz, pitch,
                                                       g.idx2, g.idy2, g.idz2, g.diag, d_dot);
        cudaDeviceSynchronize();
        float pAp = gpu_sum_f(d_dot, n3d, d_scalar);
        if (pAp < 1e-15f)
            break;
        float alpha = rsold / pAp;
        axpy_kernel_f<<<grid3d, block3d>>>(p, d_p, alpha, g.solid, nx, ny, nz, pitch);
        axpy_dot_kernel_f<<<grid3d, block3d>>>(d_r, d_Ap, -alpha, g.solid, nx, ny, nz, pitch,
                                               d_dot);
        cudaDeviceSynchronize();
        float rsnew = gpu_sum_f(d_dot, n3d, d_scalar);
        if (sqrtf(rsnew) < tol)
            break;
        precond_.apply_optimized(g, d_r, d_z);
        ...
        dot_kernel_f<<<nb1d, 256>>>(d_r, d_z, g.solid, N, d_dot);
        cudaDeviceSynchronize();
        float rz = gpu_sum_f(d_dot, nb1d, d_scalar), beta = rz / rsold;
        rsold = rz;
        cudaMemcpy(d_Ap, d_p, N * sizeof(float), cudaMemcpyDeviceToDevice);
        cudaMemcpy(d_p, d_z, N * sizeof(float), cudaMemcpyDeviceToDevice);
        axpy_kernel_f<<<grid3d, block3d>>>(d_p, d_Ap, beta, g.solid, nx, ny, nz, pitch);
        cudaDeviceSynchronize();
    }
    return rsold;
}
...
int main() {
    ...
    for (auto& B : grids) {
        ...
        std::vector<float> h_rhs(N, 0.0f);
        std::vector<char> h_solid(N, 0);
        for (int i = 1; i <= nx; i++)
            for (int j = 1; j <= ny; j++)
                for (int k = 1; k <= nz; k++) {
                    int id    = i + j * (nx + 2) + k * (nx + 2) * (ny + 2);
                    h_rhs[id] = ((float)i / nx + (float)j / ny + (float)k / nz) * 0.5f +
                                ((rand() % 1000) / 1000.0f - 0.5f) * 0.5f;
                }
        // Double opt
        CudaGrid3D gd;
        gd.allocate(nx, ny, nz, 1.0 / nx, 1.0 / ny, 1.0 / nz);
        ...
        std::vector<double> hrd(N);
        for (int i = 0; i < N; i++)
            hrd[i] = h_rhs[i];
        cudaMemcpy(dr, hrd.data(), N * sizeof(double), cudaMemcpyHostToDevice);
        CudaPCG3D pcg;
        for (int w = 0; w < 2; w++) {
            cudaMemset(dp, 0, N * sizeof(double));
            pcg.solve_optimized(gd, dp, dr, 5, 1e-6f);
        }
        cudaDeviceSynchronize();
        cudaMemset(dp, 0, N * sizeof(double));
        cudaMemcpy(dr, hrd.data(), N * sizeof(double), cudaMemcpyHostToDevice);
        auto t0 = std::chrono::high_resolution_clock::now();
        pcg.solve_optimized(gd, dp, dr, B.iters, 1e-6f);
        cudaDeviceSynchronize();
        auto t1     = std::chrono::high_resolution_clock::now();
        double d_ms = std::chrono::duration<double>(t1 - t0).count() * 1000.0;
        // Float opt
        CudaGrid3Df gf;
        gf.allocate(nx, ny, nz, 1.0f / nx, 1.0f / ny, 1.0f / nz);
        cudaMemcpy(gf.solid, h_solid.data(), N * sizeof(bool), cudaMemcpyHostToDevice);
        float *fp, *fr;
        cudaMalloc(&fp, N * sizeof(float));
        cudaMalloc(&fr, N * sizeof(float));
        cudaMemcpy(fr, h_rhs.data(), N * sizeof(float), cudaMemcpyHostToDevice);
        CudaPCG3Df pcgf;
        for (int w = 0; w < 2; w++) {
            cudaMemset(fp, 0, N * sizeof(float));
            pcgf.solve_optimized(gf, fp, fr, 5, 1e-6f);
        }
        cudaDeviceSynchronize();
        cudaMemset(fp, 0, N * sizeof(float));
        cudaMemcpy(fr, h_rhs.data(), N * sizeof(float), cudaMemcpyHostToDevice);
        auto t2 = std::chrono::high_resolution_clock::now();
        pcgf.solve_optimized(gf, fp, fr, B.iters, 1e-6f);
        cudaDeviceSynchronize();
