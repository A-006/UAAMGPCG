...
// ═══════════════════════════════════════════════════════════════
__global__ __launch_bounds__(512, 2) void rbgs_tiled_kernel_f(float* __restrict x,
                                                              const float* __restrict b,
                                                              const bool* __restrict solid, int nx,
                                                              int ny, int nz, int pitch, float idx2,
                                                              float idy2, float idz2, float diag) {
    ...
    if (tx == 0) {
        int gl = gi - 1;
        if (gl >= 1 && gj <= ny && gk <= nz) {
            int h         = opti_idx_f(gl, gj, gk, pitch, ny);
            sx[0][lj][lk] = x[h];
            ss[0][lj][lk] = solid[h];
        } else {
            ss[0][lj][lk] = true;
    ...
    if (tx == TILED_T - 1) {
        ...
        if (gr <= nx && gj <= ny && gk <= nz) {
            int h                   = opti_idx_f(gr, gj, gk, pitch, ny);
            sx[TILED_T + 1][lj][lk] = x[h];
            ss[TILED_T + 1][lj][lk] = solid[h];
        } else {
            ss[TILED_T + 1][lj][lk] = true;
            sx[TILED_T + 1][lj][lk] = 0.0f;
        }
    }
    if (ty == 0) {
        int gb = gj - 1;
        if (gi <= nx && gb >= 1 && gk <= nz) {
            int h         = opti_idx_f(gi, gb, gk, pitch, ny);
            sx[li][0][lk] = x[h];
            ss[li][0][lk] = solid[h];
        } else {
            ss[li][0][lk] = true;
    ...
    if (ty == TILED_T - 1) {
        ...
        if (gi <= nx && gt <= ny && gk <= nz) {
            int h                   = opti_idx_f(gi, gt, gk, pitch, ny);
            sx[li][TILED_T + 1][lk] = x[h];
            ss[li][TILED_T + 1][lk] = solid[h];
        } else {
            ss[li][TILED_T + 1][lk] = true;
            sx[li][TILED_T + 1][lk] = 0.0f;
        }
    }
    if (tz == 0) {
        int gf = gk - 1;
        if (gi <= nx && gj <= ny && gf >= 1) {
            int h         = opti_idx_f(gi, gj, gf, pitch, ny);
            sx[li][lj][0] = x[h];
            ss[li][lj][0] = solid[h];
        } else {
            ss[li][lj][0] = true;
...
__global__ __launch_bounds__(512, 2) void rbgs_smooth_only_kernel_f(
    float* __restrict x, const float* __restrict b, const bool* __restrict solid, int nx, int ny,
    int nz, int pitch, float idx2, float idy2, float idz2, float diag) {
    __shared__ float sx[TILED_TH][TILED_TH][TILED_TH];
    __shared__ bool ss[TILED_TH][TILED_TH][TILED_TH];
...
__global__ __launch_bounds__(512, 2) void rbgs_restrict_aggregated_kernel_f(
    ...
    float* __restrict b_coarse, const bool* __restrict solid_coarse, int nx, int ny, int nz,
    int pitch, float idx2, float idy2, float idz2, float diag, int cnx, int cny, int cpitch) {
    __shared__ float sx[TILED_TH][TILED_TH][TILED_TH];
    __shared__ bool ss[TILED_TH][TILED_TH][TILED_TH];
    ...
    if (valid && !ss[li][lj][lk]) {
        ...
        if (!solid_coarse[cid])
            atomicAdd(&b_coarse[cid], res / 8.0f);
        x[gid] = sx[li][lj][lk];
    }
}
...
__global__ __launch_bounds__(512, 2) void prolong_rbgs_aggregated_kernel_f(
    float* __restrict x_fine, const float* __restrict x_coarse, const float* __restrict b,
    const bool* __restrict solid, int fnx, int fny, int fnz, int fpitch, float idx2, float idy2,
    float idz2, float diag, int cny, int cpitch) {
    __shared__ float sx[TILED_TH][TILED_TH][TILED_TH];
    __shared__ bool ss[TILED_TH][TILED_TH][TILED_TH];
    ...
    if (tx == 0) {
        ...
        if (gl >= 1 && gj <= fny && gk <= fnz) {
            int h         = opti_idx_f(gl, gj, gk, fpitch, fny);
            sx[0][lj][lk] = x_fine[h];
            ss[0][lj][lk] = solid[h];
        } else {
            ss[0][lj][lk] = true;
    ...
    if (tx == TILED_T - 1) {
        ...
        if (gr <= fnx && gj <= fny && gk <= fnz) {
            int h                   = opti_idx_f(gr, gj, gk, fpitch, fny);
    ...
    if (ty == 0) {
        ...
        if (gi <= fnx && gb >= 1 && gk <= fnz) {
            int h         = opti_idx_f(gi, gb, gk, fpitch, fny);
            sx[li][0][lk] = x_fine[h];
            ss[li][0][lk] = solid[h];
        } else {
            ss[li][0][lk] = true;
    ...
    if (ty == TILED_T - 1) {
        ...
        if (gi <= fnx && gt <= fny && gk <= fnz) {
            int h                   = opti_idx_f(gi, gt, gk, fpitch, fny);
    ...
    if (tz == 0) {
        ...
        if (gi <= fnx && gj <= fny && gf >= 1) {
            int h         = opti_idx_f(gi, gj, gf, fpitch, fny);
            sx[li][lj][0] = x_fine[h];
            ss[li][lj][0] = solid[h];
        } else {
            ss[li][lj][0] = true;
    ...
    if (tz == TILED_T - 1) {
        ...
        if (gi <= fnx && gj <= fny && gk2 <= fnz) {
            int h                   = opti_idx_f(gi, gj, gk2, fpitch, fny);
    ...
    if (valid && !ss[li][lj][lk]) {
        int ic = (gi + 1) / 2, jc = (gj + 1) / 2, kc = (gk + 1) / 2;
        sx[li][lj][lk] += x_coarse[opti_idx_f(ic, jc, kc, cpitch, cny)]; // injection weight 1.0
    }
    __syncthreads();
...
// ── Utility kernels ──
__global__ void restrict_opt_kernel_f(const float* x, const float* b, const bool* solid,
                                      float* b_coarse, bool* solid_coarse, int fnx, int fny,
                                      int fnz, int fpitch, float idx2, float idy2, float idz2,
                                      float diag, int cstride) {
        ...
        for (int dj = 0; dj < 2; dj++)
            for (int dk = 0; dk < 2; dk++) {
                ...
                if (solid[fidx])
                    continue;
                float pC  = x[fidx];
                float pL  = (fi > 1 && !solid[opti_idx_f(fi - 1, fj, fk, fpitch, fny)])
                                ? x[opti_idx_f(fi - 1, fj, fk, fpitch, fny)]
                                : pC;
                float pR  = (fi < fnx && !solid[opti_idx_f(fi + 1, fj, fk, fpitch, fny)])
                                ? x[opti_idx_f(fi + 1, fj, fk, fpitch, fny)]
                                : pC;
                float pB  = (fj > 1 && !solid[opti_idx_f(fi, fj - 1, fk, fpitch, fny)])
                                ? x[opti_idx_f(fi, fj - 1, fk, fpitch, fny)]
                                : pC;
                float pT  = (fj < fny && !solid[opti_idx_f(fi, fj + 1, fk, fpitch, fny)])
                                ? x[opti_idx_f(fi, fj + 1, fk, fpitch, fny)]
                                : pC;
                float pF  = (fk > 1 && !solid[opti_idx_f(fi, fj, fk - 1, fpitch, fny)])
                                ? x[opti_idx_f(fi, fj, fk - 1, fpitch, fny)]
                                : pC;
                float pK  = (fk < fnz && !solid[opti_idx_f(fi, fj, fk + 1, fpitch, fny)])
                                ? x[opti_idx_f(fi, fj, fk + 1, fpitch, fny)]
                                : pC;
                float lap = (pL + pR) * idx2 + (pB + pT) * idy2 + (pF + pK) * idz2;
...
__global__ void prolong_opt_kernel_f(float* x_fine, const float* x_coarse, const bool* solid_fine,
    ...
    x_fine[fid] += x_coarse[opti_idx_f(ic, jc, kc, cstride, fny / 2)]; // injection weight 1.0
}

__global__ void zero_kernel_opt_f(float* a, int N) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < N)
        a[i] = 0.0f;
}
__global__ void copy_kernel_opt_f(float* dst, const float* src, int N) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < N)
        dst[i] = src[i];
}
__global__ void restrict_solid_opt_kernel_f(const bool* sf, bool* sc, int fnx, int fny, int fnz,
...
// ═══════════════════════════════════════════════════════════════
static void vCycle_opt_f(CudaUAAMGPreconditioner3Df::Level* levels, int lv, int nl,
                         cudaStream_t stream) {
    auto& L = levels[lv];
    ...
    if (lv == nl - 1) {
        rbgs_coarsest_kernel_f<<<grid, block, 0, stream>>>(
    ...
    // V(2,2): 1 extra smooth-only + fused smooth+restrict; 1 fused prolong+smooth + 1 extra
    // smooth-only
    ...
    rbgs_restrict_aggregated_kernel_f<<<grid, block, 0, stream>>>(
        L.g.x, L.g.b, L.g.solid, coarse.g.b, coarse.g.solid, nx, ny, nz, L.g.pitch, L.g.idx2,
        L.g.idy2, L.g.idz2, L.g.diag, cnx, cny, coarse.g.pitch);

    vCycle_opt_f(levels, lv + 1, nl, stream);

    prolong_rbgs_aggregated_kernel_f<<<grid, block, 0, stream>>>(
        L.g.x, coarse.g.x, L.g.b, L.g.solid, nx, ny, nz, L.g.pitch, L.g.idx2, L.g.idy2, L.g.idz2,
        L.g.diag, cny, coarse.g.pitch);
    for (int s = 0; s < NU - 1; s++) {
        rbgs_smooth_only_kernel_f<<<grid, block, 0, stream>>>(
            L.g.x, L.g.b, L.g.solid, nx, ny, nz, L.g.pitch, L.g.idx2, L.g.idy2, L.g.idz2, L.g.diag);
    }
...
void CudaUAAMGPreconditioner3Df::apply_optimized(const CudaGrid3Df& fine, const float* r,
                                                 float* z) {
    this->build(fine);
    int nl = (int)levels_.size();
    if (nl == 0)
    ...
    for (int l = 1; l < nl; l++) {
        ...
        dim3 block(TILED_T, TILED_T, TILED_T);
        dim3 grid((cL.g.nx + TILED_T - 1) / TILED_T, (cL.g.ny + TILED_T - 1) / TILED_T,
                  (cL.g.nz + TILED_T - 1) / TILED_T);
        restrict_solid_opt_kernel_f<<<grid, block, 0, stream>>>(
            fL.g.solid, cL.g.solid, fL.g.nx, fL.g.ny, fL.g.nz, fL.g.pitch, cL.g.nx, cL.g.ny,
            cL.g.nz, cL.g.pitch);
    }
