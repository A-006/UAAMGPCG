...
__device__ inline int pidx(int i, int j, int k, int nx, int ny) {
...
__device__ inline int tidx(int i, int j, int k, int ntx, int nty) {
    ...
    return (((tz * nty + ty) * ntx + tx) << 9) | (wz << 6) | (wy << 3) |
           wx; // *512 + wz*64 + wy*8 + wx
}
...
__global__ void rbgs_plain(double* x, const double* b, int nx, int ny, int nz, int parity) {
    ...
    if (((i + j + k) & 1) != parity)
        return;
    int id   = pidx(i, j, k, nx, ny);
    double c = x[id];
    double s = (i > 0 ? x[pidx(i - 1, j, k, nx, ny)] : c) +
               (i < nx - 1 ? x[pidx(i + 1, j, k, nx, ny)] : c) +
               (j > 0 ? x[pidx(i, j - 1, k, nx, ny)] : c) +
               (j < ny - 1 ? x[pidx(i, j + 1, k, nx, ny)] : c) +
               (k > 0 ? x[pidx(i, j, k - 1, nx, ny)] : c) +
               (k < nz - 1 ? x[pidx(i, j, k + 1, nx, ny)] : c);
    x[id]    = (b[id] + s) / 6.0;
}

__global__ void rbgs_tiled(double* x, const double* b, int nx, int ny, int nz, int ntx, int nty,
                           int parity) {
    int i = blockIdx.x * blockDim.x + threadIdx.x, j = blockIdx.y * blockDim.y + threadIdx.y,
        k = blockIdx.z * blockDim.z + threadIdx.z;
    if (i >= nx || j >= ny || k >= nz)
    ...
    if (((i + j + k) & 1) != parity)
        return;
    int id   = tidx(i, j, k, ntx, nty);
    double c = x[id];
    double s = (i > 0 ? x[tidx(i - 1, j, k, ntx, nty)] : c) +
               (i < nx - 1 ? x[tidx(i + 1, j, k, ntx, nty)] : c) +
               (j > 0 ? x[tidx(i, j - 1, k, ntx, nty)] : c) +
               (j < ny - 1 ? x[tidx(i, j + 1, k, ntx, nty)] : c) +
               (k > 0 ? x[tidx(i, j, k - 1, ntx, nty)] : c) +
               (k < nz - 1 ? x[tidx(i, j, k + 1, ntx, nty)] : c);
    x[id]    = (b[id] + s) / 6.0;
}
...
static void bench(int nx, int ny, int nz) {
    ...
    // plain
    for (int w = 0; w < 10; w++) {
        rbgs_plain<<<grd, blk>>>(xp, bp, nx, ny, nz, 1);
        rbgs_plain<<<grd, blk>>>(xp, bp, nx, ny, nz, 0);
    }
    ...
    // tiled
    for (int w = 0; w < 10; w++) {
        rbgs_tiled<<<grd, blk>>>(xt, bt, nx, ny, nz, ntx, nty, 1);
        rbgs_tiled<<<grd, blk>>>(xt, bt, nx, ny, nz, ntx, nty, 0);
    }
    cudaDeviceSynchronize();
