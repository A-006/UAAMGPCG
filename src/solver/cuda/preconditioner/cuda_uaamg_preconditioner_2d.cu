 ...
 */
#include "solver/cuda/preconditioner/cuda_uaamg_preconditioner_2d.h"
...
__global__ void setup_fine_coeffs_kernel(const bool* solid, double* cx, double* cy, int nx, int ny,
                                         int stride, double idx2, double idy2) {
...
__global__ void setup_diag_kernel(const bool* solid, const double* cx, const double* cy,
                                  double* diag, int nx, int ny, int stride) {
    int i = blockIdx.x * blockDim.x + threadIdx.x + 1;
    int j = blockIdx.y * blockDim.y + threadIdx.y + 1;
    if (i > nx || j > ny)
        return;
...
__global__ void galerkin_coeffs_kernel(const double* fcx, const double* fcy, const bool* csolid,
                                       double* ccx, double* ccy, int fstride, int cnx, int cny,
                                       int cstride) {
    ...
    for (int dj = 0; dj < 2; dj++)
        sx += fcx[dev_idx(i_f + 1, j_f + dj, fstride)]; // +x edge = fine i_f+1
    for (int di = 0; di < 2; di++)
        sy += fcy[dev_idx(i_f + di, j_f + 1, fstride)];
    ccx[cid] = sx;
    ccy[cid] = sy;
}

// ── RBGS sweep (one parity) using stored coefficients ──
__global__ void rbgs_coeff_kernel(double* x, const double* b, const bool* solid, const double* diag,
                                  const double* cx, const double* cy, int nx, int ny, int stride,
                                  int parity) {
    int i = blockIdx.x * blockDim.x + threadIdx.x + 1;
    int j = blockIdx.y * blockDim.y + threadIdx.y + 1;
    if (i > nx || j > ny)
...
__global__ void restrict_coeff_kernel(const double* xf, const double* bf, const bool* fsolid,
                                      const double* fdiag, const double* fcx, const double* fcy,
                                      double* bc, const bool* csolid, int fnx, int fny, int fstride,
                                      int cnx, int cny, int cstride) {
    int ic = blockIdx.x * blockDim.x + threadIdx.x + 1;
    int jc = blockIdx.y * blockDim.y + threadIdx.y + 1;
    if (ic > cnx || jc > cny)
        return;
    ...
    for (int di = 0; di < 2; di++)
        for (int dj = 0; dj < 2; dj++) {
            ...
            double Ax = fdiag[fid] * xf[fid] - fcx[fid] * xf[dev_idx(fi + 1, fj, fstride)] -
                        fcx[im] * xf[im] - fcy[fid] * xf[dev_idx(fi, fj + 1, fstride)] -
                        fcy[jm] * xf[jm];
            sum += bf[fid] - Ax;
        }
    bc[cid] = sum;
...
__global__ void restrict_solid_kernel(const bool* solid_fine, bool* solid_coarse, int fnx, int fny,
                                      int fstride, int cstride) {
    int ic  = blockIdx.x * blockDim.x + threadIdx.x + 1;
    int jc  = blockIdx.y * blockDim.y + threadIdx.y + 1;
    ...
    for (int di = 0; di < 2; di++)
        for (int dj = 0; dj < 2; dj++)
            if (solid_fine[dev_idx(i_f + di, j_f + dj, fstride)])
                sc++;
...
static void rbgs_sweep(CudaUAAMGPreconditioner::Level& L, cudaStream_t stream, bool reverse) {
    ...
    int p0 = reverse ? 0 : 1;
    rbgs_coeff_kernel<<<grid, block, 0, stream>>>(L.g.x, L.g.b, L.g.solid, L.diag, L.cx, L.cy, nx,
                                                  ny, L.g.pitch, p0);
    rbgs_coeff_kernel<<<grid, block, 0, stream>>>(L.g.x, L.g.b, L.g.solid, L.diag, L.cx, L.cy, nx,
                                                  ny, L.g.pitch, 1 - p0);
}
...
static void vCycle(CudaUAAMGPreconditioner::Level* levels, int level, int nlevels,
    ...
    if (level == nlevels - 1) {
        for (int s = 0; s < 10; s++) {
            rbgs_sweep(L, stream, false);
            rbgs_sweep(L, stream, true);
        }
        return;
    }
    ...
    restrict_coeff_kernel<<<cgrid, cblock, 0, stream>>>(L.g.x, L.g.b, L.g.solid, L.diag, L.cx, L.cy,
                                                        coarse.g.b, coarse.g.solid, nx, ny,
                                                        L.g.pitch, cnx, cny, coarse.g.pitch);
    ...
    prolong_kernel<<<fgrid, fblock, 0, stream>>>(L.g.x, coarse.g.x, L.g.solid, nx, ny, L.g.pitch,
                                                 coarse.g.pitch);
...
void CudaUAAMGPreconditioner::build(const CudaGrid& fine) {
    if (cached_nx_ == fine.nx && cached_ny_ == fine.ny)
        return;
    destroy();
    ...
    while (nx >= 2 && ny >= 2) {
        ...
        L.stride = nx + 2;
        int N    = (nx + 2) * (ny + 2);
        cudaMalloc(&L.diag, N * sizeof(double));
        cudaMemset(L.diag, 0, N * sizeof(double));
        cudaMalloc(&L.cx, N * sizeof(double));
        cudaMemset(L.cx, 0, N * sizeof(double));
        cudaMalloc(&L.cy, N * sizeof(double));
        cudaMemset(L.cy, 0, N * sizeof(double));
...
void CudaUAAMGPreconditioner::apply(const CudaGrid& fine, const double* r, double* z) {
    ...
    for (int l = 1; l < nl; l++) {
        ...
        restrict_solid_kernel<<<grid, block, 0, stream>>>(fL.g.solid, cL.g.solid, fL.g.nx, fL.g.ny,
                                                          fL.g.pitch, cL.g.pitch);
    }
    ...
    // Stencil coefficients: finest from solid, coarse via Galerkin.
    {
        ...
        int nx = L.g.nx, ny = L.g.ny;
        dim3 block(16, 16), grid((nx + 15) / 16, (ny + 15) / 16);
        setup_fine_coeffs_kernel<<<grid, block, 0, stream>>>(L.g.solid, L.cx, L.cy, nx, ny,
                                                             L.g.pitch, L.g.idx2, L.g.idy2);
        setup_diag_kernel<<<grid, block, 0, stream>>>(L.g.solid, L.cx, L.cy, L.diag, nx, ny,
                                                      L.g.pitch);
    }
    for (int l = 1; l < nl; l++) {
        auto& fL = levels_[l - 1];
        ...
        galerkin_coeffs_kernel<<<grid, block, 0, stream>>>(fL.cx, fL.cy, cL.g.solid, cL.cx, cL.cy,
                                                           fL.g.pitch, cnx, cny, cL.g.pitch);
        setup_diag_kernel<<<grid, block, 0, stream>>>(cL.g.solid, cL.cx, cL.cy, cL.diag, cnx, cny,
                                                      cL.g.pitch);
    }
