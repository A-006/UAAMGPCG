...
__global__ void rbgs_pass1_kernel(double* x, const double* b, const bool* solid, int nx, int ny,
                                  int stride, double idx2, double idy2, double diag) {
    ...
    double eff_d = diag;
    if (i == 1 || solid[dev_idx(i - 1, j, stride)])
        eff_d -= idx2;
    if (i == nx || solid[dev_idx(i + 1, j, stride)])
        eff_d -= idx2;
    if (j == 1 || solid[dev_idx(i, j - 1, stride)])
        eff_d -= idy2;
    if (j == ny || solid[dev_idx(i, j + 1, stride)])
        eff_d -= idy2;
    x[id] += (eff_d < 1e-15 ? 0.0 : 1.0 / eff_d) * (b[id] - diag * pC + lap);
}

__global__ void rbgs_pass2_kernel(double* x, const double* b, const bool* solid, int nx, int ny,
                                  int stride, double idx2, double idy2, double diag) {
    int i = blockIdx.x * blockDim.x + threadIdx.x + 1;
    int j = blockIdx.y * blockDim.y + threadIdx.y + 1;
    if (i > nx || j > ny)
...
__global__ void restrict_kernel(const double* x_fine, const double* b_fine, const bool* solid_fine,
                                double* b_coarse, bool* solid_coarse, int fnx, int fny, int fstride,
                                int cstride, double idx2, double idy2, double diag) {
    ...
    for (int di = 0; di < 2; di++)
        for (int dj = 0; dj < 2; dj++) {
            ...
            double pL  = (fi > 1 && !solid_fine[dev_idx(fi - 1, fj, fstride)])
                             ? x_fine[dev_idx(fi - 1, fj, fstride)]
                             : pC;
            double pR  = (fi < fnx && !solid_fine[dev_idx(fi + 1, fj, fstride)])
                             ? x_fine[dev_idx(fi + 1, fj, fstride)]
                             : pC;
            double pB  = (fj > 1 && !solid_fine[dev_idx(fi, fj - 1, fstride)])
                             ? x_fine[dev_idx(fi, fj - 1, fstride)]
                             : pC;
            double pT  = (fj < fny && !solid_fine[dev_idx(fi, fj + 1, fstride)])
                             ? x_fine[dev_idx(fi, fj + 1, fstride)]
                             : pC;
            double lap = (pL + pR) * idx2 + (pB + pT) * idy2;
...
__global__ void prolong_kernel(double* x_fine, const double* x_coarse, const bool* solid_fine,
...
__global__ void matvec_kernel(const double* p, double* Ap, const bool* solid, int nx, int ny,
                              int stride, double idx2, double idy2, double diag) {
    int i = blockIdx.x * blockDim.x + threadIdx.x + 1;
    int j = blockIdx.y * blockDim.y + threadIdx.y + 1;
    if (i > nx || j > ny)
...
__global__ void dot_partial_kernel(const double* a, const double* b, const bool* solid, int N,
                                   double* d_partial) {
...
static void cpu_rbgs_sweep(double* x, const double* b, const bool* solid, int nx, int ny,
                           int stride, double idx2, double idy2, double diag) {
    ...
    for (int i = 1; i <= nx; i++)
        for (int j = 1 + (i % 2); j <= ny; j += 2) {
            ...
            double lap   = (pL + pR) * idx2 + (pB + pT) * idy2;
            double eff_d = diag;
            if (i == 1 || solid[(i - 1) + j * stride])
                eff_d -= idx2;
            if (i == nx || solid[(i + 1) + j * stride])
                eff_d -= idx2;
            if (j == 1 || solid[i + (j - 1) * stride])
                eff_d -= idy2;
            if (j == ny || solid[i + (j + 1) * stride])
                eff_d -= idy2;
            x[id] += (eff_d < 1e-15 ? 0.0 : 1.0 / eff_d) * (b[id] - diag * pC + lap);
        }
...
static void cpu_restrict(const double* x, const double* b, const bool* solid, double* b_c, int fnx,
                         int fny, int fs, int cs, double idx2, double idy2, double diag) {
        ...
        for (int jc = 1; jc <= fny / 2; jc++) {
            ...
            for (int di = 0; di < 2; di++)
                for (int dj = 0; dj < 2; dj++) {
                    ...
                    double pR =
                        (fi < fnx && !solid[(fi + 1) + fj * fs]) ? x[(fi + 1) + fj * fs] : pC;
                    double pB = (fj > 1 && !solid[fi + (fj - 1) * fs]) ? x[fi + (fj - 1) * fs] : pC;
                    double pT =
                        (fj < fny && !solid[fi + (fj + 1) * fs]) ? x[fi + (fj + 1) * fs] : pC;
                    double lap = (pL + pR) * idx2 + (pB + pT) * idy2;
...
static void cpu_prolong(double* x_f, const double* x_c, const bool* solid_f, int fnx, int fny,
                        int fs, int cs) {
    for (int i = 1; i <= fnx; i++)
        for (int j = 1; j <= fny; j++) {
            int fid = i + j * fs;
...
static void cpu_matvec(const double* p, double* Ap, const bool* solid, int nx, int ny, int stride,
...
static bool all_close(const double* a, const double* b, int n, double tol,
...
static int test_rbgs() {
    ...
    CUDA_CHECK(cudaMalloc(&d_s, N * sizeof(bool)));
    CUDA_CHECK(cudaMemcpy(d_s, h_solid.data(), N * sizeof(bool), cudaMemcpyHostToDevice));
...
static int test_matvec() {
    ...
    int nx = 8, ny = 8, stride = nx + 2, N = (nx + 2) * (ny + 2);
    double dx = 2.0, dy = 0.5, idx2 = 1.0 / (dx * dx), idy2 = 1.0 / (dy * dy),
           diag = 2.0 * (idx2 + idy2);
...
static int test_restrict() {
    ...
    int fnx = 8, fny = 8, fs = fnx + 2, cs = fnx / 2 + 2, Nf = (fnx + 2) * (fny + 2),
        Nc    = (fnx / 2 + 2) * (fny / 2 + 2);
    double dx = 1.0, dy = 1.0, idx2 = 1.0, idy2 = 1.0, diag = 4.0;
    ...
    // CPU
    cpu_restrict(h_x.data(), h_b.data(), (bool*)h_solid.data(), h_bc_cpu.data(), fnx, fny, fs, cs,
                 idx2, idy2, diag);

    // GPU
    double *d_x, *d_b, *d_bc;
    bool *d_s, *d_sc;
    CUDA_CHECK(cudaMalloc(&d_x, Nf * sizeof(double)));
    CUDA_CHECK(cudaMemcpy(d_x, h_x.data(), Nf * sizeof(double), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMalloc(&d_b, Nf * sizeof(double)));
    CUDA_CHECK(cudaMemcpy(d_b, h_b.data(), Nf * sizeof(double), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMalloc(&d_bc, Nc * sizeof(double)));
    CUDA_CHECK(cudaMemset(d_bc, 0, Nc * sizeof(double)));
    CUDA_CHECK(cudaMalloc(&d_s, Nf * sizeof(bool)));
    CUDA_CHECK(cudaMemcpy(d_s, h_solid.data(), Nf * sizeof(bool), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMalloc(&d_sc, Nc * sizeof(bool)));
    CUDA_CHECK(cudaMemset(d_sc, 0, Nc * sizeof(bool)));
...
static int test_prolong() {
    ...
    int cnx = 4, cny = 4, fnx = 8, fny = 8, cs = cnx + 2, fs = fnx + 2, Nf = (fnx + 2) * (fny + 2),
        Nc = (cnx + 2) * (cny + 2);
    ...
    double *d_xf, *d_xc;
    bool* d_s;
    CUDA_CHECK(cudaMalloc(&d_xf, Nf * sizeof(double)));
    CUDA_CHECK(cudaMemcpy(d_xf, h_xf.data(), Nf * sizeof(double), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMalloc(&d_xc, Nc * sizeof(double)));
    CUDA_CHECK(cudaMemcpy(d_xc, h_xc.data(), Nc * sizeof(double), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMalloc(&d_s, Nf * sizeof(bool)));
    CUDA_CHECK(cudaMemcpy(d_s, h_solid.data(), Nf * sizeof(bool), cudaMemcpyHostToDevice));
...
static int test_dot() {
    ...
    CUDA_CHECK(
        cudaMemcpy(h_partial.data(), d_partial, nblocks * sizeof(double), cudaMemcpyDeviceToHost));
    double gpu_sum = 0;
    for (double v : h_partial)
        gpu_sum += v;

    double d = std::abs(cpu_sum - gpu_sum);
    printf("  cpu=%.15e gpu=%.15e diff=%e %s\n", cpu_sum, gpu_sum, d, d < 1e-14 ? "PASS" : "FAIL");
...
static int test_full_vcycle() {
    ...
    extern void launch_gpu_vcycle(double* z, const double* r, const bool* solid, int nx, int ny,
                                  double dx, double dy);
