...
void RBGS3D::solve(Grid3D& g, const std::vector<double>& rhs_in,
        ...
        for (int i = 1; i <= nx; i++)
            for (int j = 1; j <= ny; j++)
                for (int k = 1 + ((i+j) % 2); k <= nz; k += 2) {
                    ...
                    double pL = (i > 1 && !g.is_solid(i-1,j,k)) ? g.p_at(i-1,j,k) : g.p_at(i,j,k);
                    double pR = (i < nx && !g.is_solid(i+1,j,k)) ? g.p_at(i+1,j,k) : g.p_at(i,j,k);
                    double pB = (j > 1 && !g.is_solid(i,j-1,k)) ? g.p_at(i,j-1,k) : g.p_at(i,j,k);
                    double pT = (j < ny && !g.is_solid(i,j+1,k)) ? g.p_at(i,j+1,k) : g.p_at(i,j,k);
                    double pF = (k > 1 && !g.is_solid(i,j,k-1)) ? g.p_at(i,j,k-1) : g.p_at(i,j,k);
                    double pK = (k < nz && !g.is_solid(i,j,k+1)) ? g.p_at(i,j,k+1) : g.p_at(i,j,k);
