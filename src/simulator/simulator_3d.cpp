...
ChorinSimulator3D::ChorinSimulator3D(const Config& cfg, std::unique_ptr<Solver3D> solver)
    : cfg_(cfg), grid_(cfg.NX, cfg.NY, cfg.NZ, cfg.Lx, cfg.Ly, cfg.Lz),
      prev_(cfg.NX, cfg.NY, cfg.NZ, cfg.Lx, cfg.Ly, cfg.Lz), solver_(std::move(solver)),
        ...
        for (int j = 1; j <= ny; j++)
            for (int i = 1; i < nx; i++) {
                ...
                grid_.u_at(i, j, k) += cfg_.dt * nu *
                                       ((uL + uR - 2 * uC) * idx2 + (uB + uT - 2 * uC) * idy2 +
                                        (uF + uK - 2 * uC) * idz2);
            }
    // v
    for (int k = 1; k <= nz; k++)
        for (int j = 1; j < ny; j++)
            for (int i = 1; i <= nx; i++) {
                ...
                grid_.v_at(i, j, k) += cfg_.dt * nu *
                                       ((vL + vR - 2 * vC) * idx2 + (vB + vT - 2 * vC) * idy2 +
                                        (vF + vK - 2 * vC) * idz2);
            }
    // w
    for (int k = 1; k < nz; k++)
        for (int j = 1; j <= ny; j++)
            for (int i = 1; i <= nx; i++) {
                ...
                grid_.w_at(i, j, k) += cfg_.dt * nu *
                                       ((wL + wR - 2 * wC) * idx2 + (wB + wT - 2 * wC) * idy2 +
                                        (wF + wK - 2 * wC) * idz2);
            }
}
