...
CylinderForce computeForce(const Grid& g, double dt, double U_inf, double Re, double, double,
                           double) {
    CylinderForce f;
    int nx = g.nx, ny = g.ny;
    ...
    for (int i = 0; i <= nx; i++) {
        for (int j = 1; j <= ny; j++) {
            ...
            if (i < nx && g.is_solid(i + 1, j) && !g.is_solid(i, j)) {
                ...
                double dv_dx =
                    (g.v_at(i + 1, j) - g.v_at(i, j) + g.v_at(i + 1, j - 1) - g.v_at(i, j - 1)) /
                    (2.0 * dx);
    ...
    for (int i = 1; i <= nx; i++) {
        for (int j = 0; j <= ny; j++) {
            ...
            if (j < ny && g.is_solid(i, j + 1) && !g.is_solid(i, j)) {
                ...
                double du_dy =
                    (g.u_at(i, j + 1) - g.u_at(i, j) + g.u_at(i - 1, j + 1) - g.u_at(i - 1, j)) /
                    (2.0 * dy);
