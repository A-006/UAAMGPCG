#include "io/3d/wind_turbine.h"
#include "io/3d/delta_wing.h" // reuses InflowXMin3D, OutflowXMax3D, FreeSlipYZ3D
#include <cmath>
...
bool inside_blade(const WindTurbine& wt, double xc, double yc, double zc, double angle, double& v_y,
                  double& v_z) {
    ...
    if (r < wt.hub_radius && std::abs(xc - wt.hub_center[0]) < wt.chord * 0.5) {
        v_y = -wt.angular_velocity * dz;
        v_z = wt.angular_velocity * dy;
        return true;
    }

    if (r < wt.hub_radius * 0.95)
        return false;
    if (r > wt.blade_radius)
        return false;
    if (std::abs(xc - wt.hub_center[0]) > wt.chord * 0.5)
        return false;
