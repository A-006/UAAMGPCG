...
double ScalarField3D::sample(const ScalarField3D& s, const Grid3D& g, double x, double y,
                             double z) {
...
void ScalarField3D::advect(const ScalarField3D& src, ScalarField3D& dst, const Grid3D& flow,
                           double dt) {
    for (int k = 1; k <= flow.nz; k++)
        for (int j = 1; j <= flow.ny; j++)
            for (int i = 1; i <= flow.nx; i++) {
                if (flow.is_solid(i, j, k)) {
                    dst(i, j, k) = 0.0;
                    continue;
                }
...
void apply_buoyancy(Grid3D& g, const ScalarField3D& T, double T_ref, double beta, double dt) {
    // w-face at (i-0.5)dx, (j-0.5)dy, k*dz  is between cells (i,j,k) and (i,j,k+1)
    for (int k = 1; k < g.nz; k++)
        for (int j = 1; j <= g.ny; j++)
            for (int i = 1; i <= g.nx; i++) {
                if (g.is_solid(i, j, k) || g.is_solid(i, j, k + 1))
                    continue;
