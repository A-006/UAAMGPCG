#include "numerics/geometry/cylinder_model.h"
#include <algorithm>
#include <cmath>
...
StairStepCylinder::StairStepCylinder(double cx, double cy, double R, double dx, double dy)
...
void StairStepCylinder::applyToLaplacian(Grid& g, std::vector<double>& diag,
    ...
    for (int i = 1; i <= g.nx; i++)
        for (int j = 1; j <= g.ny; j++) {
            ...
            if (i > 1 && !g.is_solid(i - 1, j))
                g.u_at(i - 1, j) = 0;
            if (i < g.nx && !g.is_solid(i + 1, j))
                g.u_at(i, j) = 0;
            if (j > 1 && !g.is_solid(i, j - 1))
                g.v_at(i, j - 1) = 0;
            if (j < g.ny && !g.is_solid(i, j + 1))
                g.v_at(i, j) = 0;
        }
}
...
SmoothCylinder::SmoothCylinder(double cx, double cy, double R, double dx, double dy, double tw)
    : cx_(cx), cy_(cy), R_(R), dx_(dx), dy_(dy), tw_(tw > 0 ? tw : dx) {}
...
void SmoothCylinder::applyToLaplacian(Grid& g, std::vector<double>& diag, std::vector<double>& ox,
