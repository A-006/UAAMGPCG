...
#include "simulator/scenarios/vortex_ring.h"
#include <iostream>
#include <iomanip>
#include <chrono>
#include <cmath>
...
int main(int argc, char** argv) {
    ...
    if (argc > 3)
        cfg.NX = cfg.NY = cfg.NZ = std::atoi(argv[3]);

    int nsteps = (int)(cfg.t_end / cfg.dt);
    std::cout << "===================================================\n";
    std::cout << "  3D Vortex Ring Connection (paper Fig. 5)\n";
    std::cout << "---------------------------------------------------\n";
    std::cout << "  Grid: " << cfg.NX << "^3   dt=" << cfg.dt << "   t_end=" << cfg.t_end
              << "   steps=" << nsteps << "\n";
    std::cout << "===================================================\n\n";
    ...
    scenarios::VortexRing ringA = {{0.40, 0.50, 0.50}, {c25, +s25, 0}, 0.14, 0.025, +0.6, 240};
    scenarios::VortexRing ringB = {{0.60, 0.50, 0.50}, {c25, -s25, 0}, 0.14, 0.025, +0.6, 240};
    ...
    std::cout << "\n  Done: " << (frame - 1) << " frames in " << std::fixed << std::setprecision(1)
