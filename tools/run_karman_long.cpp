...
int main(int argc, char** argv) {
    ...
    std::cout << "  t_end=" << cfg.t_end << "   cycles=" << nsteps << "   frames~" << nframes
              << "\n";
    ...
    for (int s = 1; s <= nsteps; s++) {
        ...
        if (s % cfg.frame_skip == 0) {
            ...
            auto force =
                computeForce(g, cfg.dt, cfg.U_inf, cfg.Re, cfg.cyl_cx, cfg.cyl_cy, cfg.cyl_R);
            auto t1        = std::chrono::high_resolution_clock::now();
            double elapsed = std::chrono::duration<double>(t1 - t0).count();
            std::cout << "  frame " << std::setw(4) << frame << "  cycle " << std::setw(5) << s
                      << "/" << nsteps << "  t=" << std::fixed << std::setprecision(2) << sim.time()
                      << "  Cl=" << std::setw(7) << std::setprecision(3) << force.Cl(cfg.U_inf, D)
                      << "  Cd=" << std::setw(7) << std::setprecision(3) << force.Cd(cfg.U_inf, D)
                      << "  |u|max=" << std::setprecision(2) << max_u << "  div=" << std::scientific
                      << std::setprecision(1) << max_div << "  elapsed=" << std::fixed
    ...
    auto t1        = std::chrono::high_resolution_clock::now();
    double elapsed = std::chrono::duration<double>(t1 - t0).count();
    std::cout << "\n  Done: " << nsteps << " cycles, " << (frame - 1) << " frames in " << std::fixed
