...
int main(int argc, char** argv) {
    ...
    std::cout << "Chorin: NX=" << cfg.NX << " NY=" << cfg.NY << " dt=" << cfg.dt
              << " t_end=" << cfg.t_end << " nsteps=" << nsteps << "  out=" << cfg.out_dir << "\n";
    ...
    for (int s = 1; s <= nsteps; s++) {
        ...
        if (s % cfg.frame_skip == 0) {
            ...
            auto F    = computeForce(sim.grid(), cfg.dt, cfg.U_inf, cfg.Re, cfg.cyl_cx, cfg.cyl_cy,
                                     cfg.cyl_R);
            auto t1   = std::chrono::high_resolution_clock::now();
            double el = std::chrono::duration<double>(t1 - t0).count();
            std::cout << "  frame " << std::setw(4) << frame << "  step " << std::setw(6) << s
