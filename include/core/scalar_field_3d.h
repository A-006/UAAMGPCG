...
public:
    ...
    static void advect(const ScalarField3D& src, ScalarField3D& dst, const Grid3D& flow, double dt);

    // Sample value at arbitrary physical position via trilinear interpolation.
    static double sample(const ScalarField3D& s, const Grid3D& mesh, double x, double y, double z);
