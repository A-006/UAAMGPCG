...
struct CylinderForce {
    ...
    double Cd(double U_inf, double D) const {
        return 2.0 * Fx / (U_inf * U_inf * D);
    }
    double Cl(double U_inf, double D) const {
        return 2.0 * Fy / (U_inf * U_inf * D);
    }
};
...
CylinderForce computeForce(const Grid& g, double dt, double U_inf, double Re, double cyl_cx,
                           double cyl_cy, double cyl_R);
...
/// St = f * D / U_inf
double estimateStrouhal(const std::vector<double>& time, const std::vector<double>& Cl,
