...
template <Layout L> class Field {
public:
    explicit Field(const Mesh2D& mesh) : mesh_(&mesh), data_(layout_size(mesh)) {}
...
private:
    ...
    static int layout_size(const Mesh2D& m) {
        if constexpr (L == Layout::Cell)
            return m.p_size();
        if constexpr (L == Layout::FaceX)
            return m.u_size();
        if constexpr (L == Layout::FaceY)
            return m.v_size();
        return 0;
    }
