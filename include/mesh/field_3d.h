...
template <Layout3D L> class Field3D {
public:
    explicit Field3D(const Mesh3D& mesh) : mesh_(&mesh), data_(layout_size(mesh)) {}
...
private:
    ...
    static int layout_size(const Mesh3D& m) {
        if constexpr (L == Layout3D::Cell)
            return m.p_size();
        if constexpr (L == Layout3D::FaceX)
            return m.u_size();
        if constexpr (L == Layout3D::FaceY)
            return m.v_size();
        if constexpr (L == Layout3D::FaceZ)
            return m.w_size();
        return 0;
    }
    int flat_index(int i, int j, int k) const {
        if constexpr (L == Layout3D::Cell)
            return mesh_->ip(i, j, k);
        if constexpr (L == Layout3D::FaceX)
            return mesh_->iu(i, j, k);
        if constexpr (L == Layout3D::FaceY)
            return mesh_->iv(i, j, k);
        if constexpr (L == Layout3D::FaceZ)
            return mesh_->iw(i, j, k);
        return 0;
    }
};
