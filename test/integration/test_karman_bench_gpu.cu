...
static std::vector<double> run_one(const Config& base_cfg, std::unique_ptr<Solver> solver,
                                   const std::string& label, int iters) {
    Config cfg      = base_cfg;
    cfg.solver      = label;
...
int main() {
    ...
    auto run = [&](std::unique_ptr<Solver> solver, const std::string& label, int iters) {
        std::cout << "  " << std::left << std::setw(20) << (label + " ...") << std::flush;
    ...
    std::cout << "| " << std::left << std::setw(20) << "Solver" << std::right << std::setw(12)
              << "Time(s)" << std::setw(12) << "ms/step" << std::setw(10) << "Iters"
              << std::setw(14) << "vs Jacobi" << std::setw(7) << ""
              << " |\n";
    std::cout << "+" << std::string(75, '-') << "+\n";
    ...
    for (const auto& r : results) {
        ...
        std::cout << "| " << std::left << std::setw(20) << r.name << std::right << std::fixed
                  << std::setprecision(3) << std::setw(12) << r.time << std::setw(12)
                  << std::setprecision(1) << ms << std::setw(10) << r.iters_per_step
                  << std::setw(11) << std::setprecision(1) << ratio << "%" << std::setw(8) << ""
                  << " |\n";
    }
    std::cout << "+" << std::string(75, '-') << "+\n\n";
