...
static void i2_shear_layer() {
    ...
    for (int i = 0; i <= g.nx; i++)
        for (int j = 1; j <= g.ny; j++) {
            ...
            g.u_at(i, j)  = u_base + eps * std::sin(kx * x) *
                                         std::exp(-((y - y0) / delta) * ((y - y0) / delta));
        }
    for (int i = 1; i <= g.nx; i++)
        for (int j = 0; j <= g.ny; j++) {
            if (g.is_solid(i, j) || g.is_solid(i, j + 1))
            ...
            double x = (i - 0.5) * g.dx, y = j * g.dy;
            g.v_at(i, j) =
                eps * std::cos(kx * x) * std::exp(-((y - y0) / delta) * ((y - y0) / delta));
        }

    // Measure v-perturbation magnitude at interface
    auto v_rms = [&](const Grid& grid) {
        double v2 = 0;
        int n = 0, j_center = (int)(y0 / grid.dy) + 1;
        for (int i = 10; i < grid.nx - 10; i++) {
            if (grid.is_solid(i, j_center))
                continue;
    ...
    std::cout << "    v_rms: initial=" << v0 << " after=" << v1 << " ratio=" << ratio
              << " max|div|=" << max_div << "\n";
...
static double run_perturbation_test(int n_steps) {
    ...
    for (int i = 0; i <= g.nx; i++)
        for (int j = 1; j <= g.ny; j++) {
            if (g.is_solid(i, j) || g.is_solid(i + 1, j))
                continue;
            g.u_at(i, j) = U0 + eps * std::sin(2.0 * M_PI * i * g.dx / cfg.Lx) *
                                    std::cos(ky * (j - 0.5) * g.dy);
        }
    for (int i = 1; i <= g.nx; i++)
        for (int j = 0; j <= g.ny; j++) {
            if (g.is_solid(i, j) || g.is_solid(i, j + 1))
                continue;
            g.v_at(i, j) =
                eps * std::cos(2.0 * M_PI * (i - 0.5) * g.dx / cfg.Lx) * std::sin(ky * j * g.dy);
        }
    ...
    auto max_v = [&](const Grid& gr) {
        ...
        for (int i = 1; i <= gr.nx; i++)
            for (int j = 1; j <= gr.ny; j++)
                if (!gr.is_solid(i, j))
                    mv = std::max(mv, std::abs(gr.v_at(i, j)));
        return mv;
    };
    double v0   = max_v(g);
    ...
    for (int c = 0; c < ncycles; c++) {
        ...
        if (c % 20 == 0 || c == ncycles - 1)
            std::cout << "    " << std::setw(4) << c << "  " << std::fixed << std::setprecision(3)
                      ...
                      << "\n";
        if (c % 10 == 0)
            VtkWriter::write(const_cast<Grid&>(sim.grid()), c / 10, vtk_cfg);
    }
