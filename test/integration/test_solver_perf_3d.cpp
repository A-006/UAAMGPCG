...
static Result run_solver(Solver3D& solver, Grid3D& g, const std::vector<double>& rhs, int iters,
                         double tol, int warmup, int measure, bool is_pcg) {
    for (int w = 0; w < warmup; w++) {
        std::fill(g.p.begin(), g.p.end(), 0.0);
        solver.solve(g, rhs, iters, tol);
    }
    std::fill(g.p.begin(), g.p.end(), 0.0);
...
int main() {
    ...
    for (auto [nx, ny, nz, tol] : configs) {
        ...
        printf("  %s\n", std::string(50, '-').c_str());
        for (auto& r : results) {
            printf("  %-18s %10.3f %12.4e %8s\n", r.name.c_str(), r.time_ms, r.l2_res,
                   r.ok ? "PASS" : "FAIL");
            std::string label = r.name + " " + std::to_string(nx) + "x" + std::to_string(ny) + "x" +
                                std::to_string(nz);
            check(r.ok, label);
        }
    }
