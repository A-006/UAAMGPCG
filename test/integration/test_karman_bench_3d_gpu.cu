...
int main() {
    ...
    auto run = [&](std::unique_ptr<Solver> solver, const std::string& label, int iters) {
        std::cout << "  " << std::left << std::setw(22) << (label + " ...") << std::flush;
    ...
    std::cout << "| " << std::left << std::setw(22) << "Solver" << std::right << std::setw(12)
    ...
    for (const auto& r : results) {
        ...
        std::cout << "| " << std::left << std::setw(22) << r.name << std::right << std::fixed
                  ...
                  << std::setw(11) << std::setprecision(1) << ratio << "%" << std::setw(7) << ""
                  << " |\n";
    }
    std::cout << "+" << std::string(75, '-') << "+\n\n";
    ...
    check(results[4].ok && results[6].ok, "GPU and CPU UAAMGPCG both pass divergence check");
