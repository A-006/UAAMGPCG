...
static void t2_velocity_interp() {
    ...
    for (int i = 0; i <= 8; i++)
        for (int j = 1; j <= 4; j++)
            g.u_at(i, j) = i * g.dx; // u = x
    for (int i = 1; i <= 8; i++)
        for (int j = 0; j <= 4; j++)
            g.v_at(i, j) = -(j * g.dy); // v = -y
...
static void t5_pullback_roundtrip() {
    ...
    double dt = cfg.dt;
    for (int s = 0; s < 4; s++)
        sim.rk4_march_forward_public(uu, vv, dt);

    fm.set_backward_identity();
    for (int s = 0; s < 4; s++)
        sim.rk4_march_backward_public(uu, vv, -dt);
...
static void t6_gauge_uniform() {
    ...
    double max_div = 0, max_u_err = 0, avg_u = 0;
    int n = 0;
...
static void t9_compare_chorin_lfm() {
    ...
    ch.step();
    double cd = 0;
    for (int i = 1; i <= ch.grid().nx; i++)
        for (int j = 1; j <= ch.grid().ny; j++)
            if (!ch.grid().is_solid(i, j))
                cd = std::max(cd, std::abs(ch.grid().divergence(i, j)));
    double cu = 0;
    for (int i = 1; i < ch.grid().nx; i++)
        for (int j = 1; j <= ch.grid().ny; j++)
    ...
    lf.step();
    double ld = 0;
    for (int i = 1; i <= lf.grid().nx; i++)
        for (int j = 1; j <= lf.grid().ny; j++)
            if (!lf.grid().is_solid(i, j))
                ld = std::max(ld, std::abs(lf.grid().divergence(i, j)));
    double lu = 0;
    for (int i = 1; i < lf.grid().nx; i++)
        for (int j = 1; j <= lf.grid().ny; j++)
    ...
    for (int s = 0; s < nsteps; s++) {
        ...
        for (int j = 1; j <= g.ny; j++)
            for (int i = 1; i <= g.nx; i++) {
                size_t k = fm.idx(i, j);
                max_F_dev =
                    std::max(max_F_dev, std::abs(fm.F00[k] - 1.0) + std::abs(fm.F10[k]) +
                                            std::abs(fm.F01[k]) + std::abs(fm.F11[k] - 1.0));
                max_T_dev =
                    std::max(max_T_dev, std::abs(fm.T00[k] - 1.0) + std::abs(fm.T10[k]) +
                                            std::abs(fm.T01[k]) + std::abs(fm.T11[k] - 1.0));
            }
        ...
        if (s % 5 == 0 || s == nsteps - 1)
            std::cout << "  " << std::setw(4) << s << "  " << std::scientific
                      << std::setprecision(2) << max_F_dev << "  " << max_T_dev << "  " << max_div
...
static void t11_rk4_shear_forward() {
    ...
    for (int j = 3; j <= g.ny - 2; j++)
        for (int i = 3; i <= g.nx - 2; i++) {
            size_t k    = fm.idx(i, j);
...
static void t12_rk4_shear_roundtrip() {
    ...
    for (int j = 1; j <= g.ny; j++)
        for (int i = 1; i <= g.nx; i++) {
            ...
            max_Tinv_err = std::max(max_Tinv_err, std::abs(I00 - 1.0) + std::abs(I11 - 1.0) +
                                                      std::abs(I01) + std::abs(I10));
        }
...
static void t15_error_correction() {
    ...
    for (int j = 1; j <= g.ny; j++)
        for (int i = 1; i <= g.nx; i++) {
            ...
            int i0    = std::max(1, std::min(g.nx, (int)cix)),
                j0    = std::max(1, std::min(g.ny, (int)ciy));
            double fx = cix - i0, fy = ciy - j0;
            int i1 = std::min(i0 + 1, g.nx), j1 = std::min(j0 + 1, g.ny);
            size_t k00 = fm.idx(i0, j0), k10 = fm.idx(i1, j0), k01 = fm.idx(i0, j1),
                   k11 = fm.idx(i1, j1);
            double msx = (1 - fx) * (1 - fy) * mx[k00] + fx * (1 - fy) * mx[k10] +
                         (1 - fx) * fy * mx[k01] + fx * fy * mx[k11];
            double msy = (1 - fx) * (1 - fy) * my[k00] + fx * (1 - fy) * my[k10] +
                         (1 - fx) * fy * my[k01] + fx * fy * my[k11];
    ...
    for (int j = 1; j <= g.ny; j++)
        for (int i = 1; i <= g.nx; i++) {
            ...
            err_before =
                std::max(err_before, std::abs(u_hat_x[k] - uc) + std::abs(u_hat_y[k] - vc));
        }
...
static void t17_lfm_vortex_advection() {
    ...
    std::cout << "    Final:   peak|w|=" << final_peak << " (" << final_peak / initial_peak * 100
              << "% preserved)"
              << " center=(" << vx1 << "," << vy1 << ")"
...
static void t18_cylinder_re20() {
    ...
    std::cout << "  Running " << nsteps << " cycles (t_end~"
              << nsteps * cfg.dt * cfg.lfm_cycle_steps << "s)...\n";

    for (int s = 0; s < nsteps; s++) {
        sim.step();
        if (s >= nsteps / 2) { // second half (post-transient)
            auto force =
                computeForce(sim.grid(), cfg.dt, U, cfg.Re, cfg.cyl_cx, cfg.cyl_cy, cfg.cyl_R);
            Cd_hist.push_back(force.Cd(U, D));
        }
    }

    double Cd_sum = 0;
    for (double cd : Cd_hist)
        Cd_sum += cd;
