...
/// Single solver run
static std::vector<double> run_one(const Config& base_cfg, const std::string& solver_key,
                                   int iters) {
    Config cfg      = base_cfg;
    ...
    std::cout << "max_div=" << std::scientific << std::setprecision(2) << max_div << "  ";
...
int main() {
    ...
    cfg.solve_tol   = 1e-6;
    cfg.frame_skip  = 100;
    cfg.out_dir     = "/tmp/karman_test_3d";

    cfg.NY     = std::max(16, cfg.NX / 4);
    cfg.dt     = 0.5 * (cfg.Lx / cfg.NX) / cfg.U_inf;
    int nsteps = (int)(cfg.t_end / cfg.dt);

    std::cout << "Grid: " << cfg.NX << "x" << cfg.NY << "  dt=" << std::fixed
              << std::setprecision(6) << cfg.dt << "  steps=" << nsteps << "  t_end=" << cfg.t_end
              << "\n\n";
    ...
    auto run = [&](const std::string& key, const std::string& label, int iters) {
        std::cout << "  " << std::left << std::setw(18) << (label + " ...") << std::flush;
        auto v  = run_one(cfg, key, iters);
        bool ok = (v[2] > 0.5);
        results.push_back({label, v[0], (int)v[1], iters, ok});
        std::cout << std::fixed << std::setprecision(3) << std::setw(8) << v[0] << " s  ("
                  << std::setprecision(1) << (v[0] / v[1] * 1000) << " ms/step)  "
                  << (ok ? "PASS" : "FAIL") << "\n";
    };

    run("jacobi", "CPU Jacobi", 2000);
    run("rbgs", "CPU RBGS", 2000);
    run("cg", "CPU CG", 200);
    run("pcg", "CPU PCG/GMG", 50);
    run("pcg_amg", "CPU PCG/AMG", 50);
    run("pcg_uaamg", "CPU PCG/UAAMG", 50);

    std::cout << "\n";
    std::cout << "+" << std::string(62, '-') << "+\n";
    std::cout << "| " << std::left << std::setw(18) << "Solver" << std::right << std::setw(10)
              << "Time(s)" << std::setw(10) << "ms/step" << std::setw(10) << "Iters"
    ...
    for (const auto& r : results) {
        ...
        std::cout << "| " << std::left << std::setw(18) << r.name << std::right << std::fixed
                  << std::setprecision(3) << std::setw(10) << r.time << std::setw(10)
                  << std::setprecision(1) << ms << std::setw(10) << r.iters_per_step
                  << std::setw(10) << std::setprecision(1) << ratio << "%" << std::setw(4) << ""
                  << " |\n";
    }
    std::cout << "+" << std::string(62, '-') << "+\n\n";
    ...
    for (size_t i = 2; i < results.size(); i++)
        check(results[i].time < results[0].time / 5, results[i].name + " faster than Jacobi");
