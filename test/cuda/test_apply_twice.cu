...
int main() {
    ...
    printf("Test A: apply with raw RHS (like test_cuda_uaamg Test 1)\n");
    {
        ...
        CudaGrid gpu_g_raw;
        gpu_g_raw.allocate(nx, ny, dx, dy);
        ...
        cudaMemcpy(dr, raw_r0.data(), N * sizeof(double), cudaMemcpyHostToDevice);
    ...
    printf("\nTest C: apply(r0) then apply(r1) on same gpu_pre\n");
    {
        ...
        for (int i = 1; i <= nx; i++)
            for (int j = 1; j <= ny; j++) {
                ...
                double pC = cz0[id];
                double pL =
                    (i > 1 && !h_solid[(i - 1) + j * stride]) ? cz0[(i - 1) + j * stride] : pC;
                double pR =
                    (i < nx && !h_solid[(i + 1) + j * stride]) ? cz0[(i + 1) + j * stride] : pC;
                double pB =
                    (j > 1 && !h_solid[i + (j - 1) * stride]) ? cz0[i + (j - 1) * stride] : pC;
                double pT =
                    (j < ny && !h_solid[i + (j + 1) * stride]) ? cz0[i + (j + 1) * stride] : pC;
