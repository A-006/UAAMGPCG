...
int main() {
    ...
    // GPU CG
    CudaGrid3D gpu_g;
    gpu_g.allocate(nx, ny, nz, 1.0 / nx, 1.0 / ny, 1.0 / nz);
    ...
    for (int iters : {1, 5, 20, 50}) {
        ...
        for (int i = 1; i <= nx; i++)
            for (int j = 1; j <= ny; j++)
                for (int k = 1; k <= nz; k++)
                    pmax =
                        std::max(pmax, std::abs(gpu_p[i + j * (nx + 2) + k * (nx + 2) * (ny + 2)]));
    ...
    double idx2 = 1.0 / (gpu_g.dx * gpu_g.dx), idy2 = 1.0 / (gpu_g.dy * gpu_g.dy),
           idz2  = 1.0 / (gpu_g.dz * gpu_g.dz);
