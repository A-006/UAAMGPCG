...
static bool test_vcycle_gpu_vs_cpu() {
    ...
    // GPU
    CudaGrid gpu_grid;
    gpu_grid.allocate(nx, ny, 1.0 / nx, 1.0 / ny);
    ...
    for (int i = 1; i <= nx; i++)
        for (int j = 1; j <= ny; j++) {
            ...
            if (d > max_diff)
                max_diff = d;
            if (std::abs(cpu_z[id]) > max_val)
                max_val = std::abs(cpu_z[id]);
        }
...
static bool test_pcg_gpu_vs_cpu() {
    ...
    for (int iters : {0, 1, 2, 5, 10, 50}) {
        ...
        for (int i = 1; i <= nx; i++)
            for (int j = 1; j <= ny; j++)
                if (std::abs(gp[i + j * (nx + 2)]) > gmax)
                    gmax = std::abs(gp[i + j * (nx + 2)]);
    ...
    for (int i = 1; i <= nx; i++)
        for (int j = 1; j <= ny; j++) {
            ...
            if (d > max_diff)
                max_diff = d;
            if (std::abs(cpu_p[id]) > max_cpu)
                max_cpu = std::abs(cpu_p[id]);
        }
...
static bool test_vcycle_with_solid() {
    ...
    for (int i = 1; i <= nx; i++)
        for (int j = 1; j <= ny; j++) {
            ...
            int id = i + j * (nx + 2);
            if (std::sqrt((xi - cx) * (xi - cx) + (yj - cy) * (yj - cy)) < R)
                h_solid[id] = 1;
            if (!h_solid[id])
                h_r[id] = 1.0;
        }
    ...
    for (int i = 0; i <= nx + 1; i++)
        for (int j = 0; j <= ny + 1; j++)
            if (h_solid[cpu_grid.ip(i, j)])
                cpu_grid.set_solid(i, j);
    UAAMGPreconditioner cpu_precond;
    std::vector<double> cpu_z(N, 0.0);
    cpu_precond.apply(cpu_grid, h_r, cpu_z);
...
int main() {
    ...
    if (test_vcycle_gpu_vs_cpu())
        passed++;
    else
        failed++;
    if (test_pcg_gpu_vs_cpu())
        passed++;
    else
        failed++;
    if (test_vcycle_with_solid())
        passed++;
    else
        failed++;
    if (test_residual())
