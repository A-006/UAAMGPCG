...
__device__ inline int dev_idx(int i, int j, int stride) {
    return i + j * stride;
}

// RBGS pass1: odd-sum (matches CPU first checkerboard pass)
__global__ void krbgs1(double* x, const double* b, const bool* s, int nx, int ny, int stride,
    ...
    double eff_d = diag;
    if (i == 1 || s[dev_idx(i - 1, j, stride)])
        eff_d -= idx2;
    if (i == nx || s[dev_idx(i + 1, j, stride)])
        eff_d -= idx2;
    if (j == 1 || s[dev_idx(i, j - 1, stride)])
        eff_d -= idy2;
    if (j == ny || s[dev_idx(i, j + 1, stride)])
        eff_d -= idy2;
    x[id] += (eff_d < 1e-15 ? 0.0 : 1.0 / eff_d) * (b[id] - diag * pC + lap);
}

__global__ void krbgs2(double* x, const double* b, const bool* s, int nx, int ny, int stride,
                       double idx2, double idy2, double diag) {
    int i = blockIdx.x * blockDim.x + threadIdx.x + 1;
    int j = blockIdx.y * blockDim.y + threadIdx.y + 1;
    if (i > nx || j > ny)
...
__global__ void kmatvec(const double* p, double* Ap, const bool* s, int nx, int ny, int stride,
                        double idx2, double idy2, double diag) {
    int i = blockIdx.x * blockDim.x + threadIdx.x + 1;
    int j = blockIdx.y * blockDim.y + threadIdx.y + 1;
    if (i > nx || j > ny)
...
__global__ void krestrict(const double* xf, const double* bf, const bool* sf, double* bc, bool* sc,
                          int fnx, int fny, int fs, int cs, double idx2, double idy2, double diag) {
    ...
    for (int di = 0; di < 2; di++)
        for (int dj = 0; dj < 2; dj++) {
            ...
            double pR =
                (fi < fnx && !sf[dev_idx(fi + 1, fj, fs)]) ? xf[dev_idx(fi + 1, fj, fs)] : pC;
            double pB = (fj > 1 && !sf[dev_idx(fi, fj - 1, fs)]) ? xf[dev_idx(fi, fj - 1, fs)] : pC;
            double pT =
                (fj < fny && !sf[dev_idx(fi, fj + 1, fs)]) ? xf[dev_idx(fi, fj + 1, fs)] : pC;
            double lap = (pL + pR) * idx2 + (pB + pT) * idy2;
...
__global__ void kprolong(double* xf, const double* xc, const bool* sf, int fnx, int fny, int fs,
                         int cs) {
    int i = blockIdx.x * blockDim.x + threadIdx.x + 1;
    int j = blockIdx.y * blockDim.y + threadIdx.y + 1;
    if (i > fnx || j > fny)
...
static void cpu_rbgs_pass1(double* x, const double* b, const bool* s, int nx, int ny, int stride,
                           double idx2, double idy2, double diag) {
    for (int i = 1; i <= nx; i++)
        for (int j = 1 + (i % 2); j <= ny; j += 2) {
            ...
            double lap   = (pL + pR) * idx2 + (pB + pT) * idy2;
            double eff_d = diag;
            if (i == 1 || s[(i - 1) + j * stride])
                eff_d -= idx2;
            if (i == nx || s[(i + 1) + j * stride])
                eff_d -= idx2;
            if (j == 1 || s[i + (j - 1) * stride])
                eff_d -= idy2;
            if (j == ny || s[i + (j + 1) * stride])
                eff_d -= idy2;
            x[id] += (eff_d < 1e-15 ? 0.0 : 1.0 / eff_d) * (b[id] - diag * pC + lap);
        }
}

static void cpu_rbgs_pass2(double* x, const double* b, const bool* s, int nx, int ny, int stride,
                           double idx2, double idy2, double diag) {
    for (int i = 1; i <= nx; i++)
...
static void cpu_rbgs_full(double* x, const double* b, const bool* s, int nx, int ny, int stride,
                          double idx2, double idy2, double diag) {
    cpu_rbgs_pass1(x, b, s, nx, ny, stride, idx2, idy2, diag);
    cpu_rbgs_pass2(x, b, s, nx, ny, stride, idx2, idy2, diag);
}

static void cpu_matvec(const double* p, double* Ap, const bool* s, int nx, int ny, int stride,
                       double idx2, double idy2, double diag) {
    for (int i = 1; i <= nx; i++)
        for (int j = 1; j <= ny; j++) {
            int id = i + j * stride;
...
static double gpu_dot_reduce(double* d_part, int nblocks) {
    ...
    cudaMemcpy(h.data(), d_part, nblocks * sizeof(double), cudaMemcpyDeviceToHost);
    double t = 0;
    for (auto v : h)
        t += v;
    return t;
...
static int gpu_count_reduce(int* d_part, int nblocks) {
    ...
    cudaMemcpy(h.data(), d_part, nblocks * sizeof(int), cudaMemcpyDeviceToHost);
    int t = 0;
    for (auto v : h)
        t += v;
    return t;
...
static int diff_count(const double* a, const double* b, int N, double tol,
                      const bool* skip = nullptr) {
    int c = 0;
    for (int k = 0; k < N; k++)
        if ((!skip || !skip[k]) && std::abs(a[k] - b[k]) > tol)
            c++;
    return c;
}
...
struct TestCtx {
    ...
    void init() {
        ...
        for (int i = 1; i <= nx; i++)
            for (int j = 1; j <= ny; j++)
                b[i + j * stride] = std::sin(i * 0.5) * std::cos(j * 0.7);
    ...
    void to_gpu(double*& dx, double*& db, bool*& ds) {
        ...
        cudaMemcpy(db, b.data(), N * sizeof(double), cudaMemcpyHostToDevice);
        cudaMalloc(&ds, N * sizeof(bool));
        cudaMemcpy(ds, solid.data(), N * sizeof(bool), cudaMemcpyHostToDevice);
    }
};
...
static int test_rbgs_pass1(TestCtx& ctx) {
    ...
    cpu_rbgs_pass1(cpu.data(), ctx.b.data(), (bool*)ctx.solid.data(), ctx.nx, ctx.ny, ctx.stride,
                   ctx.idx2, ctx.idy2, ctx.diag);
    ...
    std::vector<double> gpu(ctx.N);
    cudaMemcpy(gpu.data(), dx, ctx.N * sizeof(double), cudaMemcpyDeviceToHost);
...
static int test_rbgs_pass2(TestCtx& ctx) {
    ...
    cpu_rbgs_pass1(cpu.data(), ctx.b.data(), (bool*)ctx.solid.data(), ctx.nx, ctx.ny, ctx.stride,
                   ctx.idx2, ctx.idy2, ctx.diag);
    cpu_rbgs_pass2(cpu.data(), ctx.b.data(), (bool*)ctx.solid.data(), ctx.nx, ctx.ny, ctx.stride,
                   ctx.idx2, ctx.idy2, ctx.diag);
...
static int test_matvec(TestCtx& ctx) {
    ...
    std::vector<double> p(ctx.N);
    for (int k = 0; k < ctx.N; k++)
        p[k] = std::sin(k * 0.1);
    std::vector<double> cpu(ctx.N);
    cpu_matvec(p.data(), cpu.data(), (bool*)ctx.solid.data(), ctx.nx, ctx.ny, ctx.stride, ctx.idx2,
               ctx.idy2, ctx.diag);

    double *dp, *dAp;
    bool* ds;
    CUDA_CHECK(cudaMalloc(&dp, ctx.N * sizeof(double)));
    CUDA_CHECK(cudaMemcpy(dp, p.data(), ctx.N * sizeof(double), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMalloc(&dAp, ctx.N * sizeof(double)));
    CUDA_CHECK(cudaMalloc(&ds, ctx.N * sizeof(bool)));
    CUDA_CHECK(cudaMemcpy(ds, ctx.solid.data(), ctx.N * sizeof(bool), cudaMemcpyHostToDevice));
    dim3 b(16, 16), g((ctx.nx + 15) / 16, (ctx.ny + 15) / 16);
    kmatvec<<<g, b>>>(dp, dAp, ds, ctx.nx, ctx.ny, ctx.stride, ctx.idx2, ctx.idy2, ctx.diag);
    cudaDeviceSynchronize();
    std::vector<double> gpu(ctx.N);
    cudaMemcpy(gpu.data(), dAp, ctx.N * sizeof(double), cudaMemcpyDeviceToHost);
    double d = max_diff(cpu.data(), gpu.data(), ctx.N, (bool*)ctx.solid.data());
    printf("  max|cpu-gpu|=%e  %s\n", d, d < 1e-14 ? "PASS" : "FAIL");
    cudaFree(dp);
    cudaFree(dAp);
...
static int test_restrict(TestCtx& ctx) {
    ...
    int fnx = ctx.nx, fny = ctx.ny, fs = ctx.stride, cnx = fnx / 2, cny = fny / 2, cs = cnx + 2,
        Nc = (cnx + 2) * (cny + 2);
    std::vector<double> xf(ctx.N), bf(ctx.N), bc_cpu(Nc, 0);
    std::vector<char> sf(ctx.N, 0), sc(Nc, 0);
    for (int i = 1; i <= fnx; i++)
        for (int j = 1; j <= fny; j++) {
            xf[i + j * fs] = std::sin(i * 0.5);
            bf[i + j * fs] = 1.0;
        }

    cpu_rbgs_full(xf.data(), bf.data(), (bool*)sf.data(), fnx, fny, fs, ctx.idx2, ctx.idy2,
                  ctx.diag);
    // Now restrict: coarse.b = avg(b_fine - A*x_fine)
    for (int ic = 1; ic <= cnx; ic++)
        for (int jc = 1; jc <= cny; jc++) {
            int if_ = 2 * ic - 1, jf_ = 2 * jc - 1;
            double sum = 0;
            int cnt    = 0;
            for (int di = 0; di < 2; di++)
                for (int dj = 0; dj < 2; dj++) {
    ...
    double *dxf, *dbf, *dbc;
    bool *dsf, *dsc;
    CUDA_CHECK(cudaMalloc(&dxf, ctx.N * sizeof(double)));
    CUDA_CHECK(cudaMemcpy(dxf, xf.data(), ctx.N * sizeof(double), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMalloc(&dbf, ctx.N * sizeof(double)));
    CUDA_CHECK(cudaMemcpy(dbf, bf.data(), ctx.N * sizeof(double), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMalloc(&dbc, Nc * sizeof(double)));
    CUDA_CHECK(cudaMemset(dbc, 0, Nc * sizeof(double)));
    CUDA_CHECK(cudaMalloc(&dsf, ctx.N * sizeof(bool)));
    CUDA_CHECK(cudaMemcpy(dsf, sf.data(), ctx.N * sizeof(bool), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMalloc(&dsc, Nc * sizeof(bool)));
    CUDA_CHECK(cudaMemset(dsc, 0, Nc * sizeof(bool)));
    ...
    cudaMemcpy(gpu.data(), dbc, Nc * sizeof(double), cudaMemcpyDeviceToHost);
...
static int test_prolong(TestCtx& ctx) {
    ...
    int cnx = ctx.nx / 2, cny = ctx.ny / 2, cs = cnx + 2, fs = ctx.stride,
        Nc = (cnx + 2) * (cny + 2);
    ...
    for (int i = 1; i <= cnx; i++)
        for (int j = 1; j <= cny; j++)
            xc[i + j * cs] = (i + j) * 0.25;
    for (int i = 1; i <= ctx.nx; i++)
        for (int j = 1; j <= ctx.ny; j++)
            if (i == 3 && j >= 2 && j <= 5)
                sf[i + j * fs] = 1;
    // CPU
    for (int i = 1; i <= ctx.nx; i++)
        for (int j = 1; j <= ctx.ny; j++) {
            ...
            if (sf[fid])
                continue;
            int ic = (i + 1) / 2, jc = (j + 1) / 2;
            xf_cpu[fid] += 2.0 * xc[ic + jc * cs];
        }
    // GPU
    double *dxf, *dxc;
    bool* dsf;
    CUDA_CHECK(cudaMalloc(&dxf, ctx.N * sizeof(double)));
    CUDA_CHECK(cudaMemset(dxf, 0, ctx.N * sizeof(double)));
    CUDA_CHECK(cudaMalloc(&dxc, Nc * sizeof(double)));
    CUDA_CHECK(cudaMemcpy(dxc, xc.data(), Nc * sizeof(double), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMalloc(&dsf, ctx.N * sizeof(bool)));
    CUDA_CHECK(cudaMemcpy(dsf, sf.data(), ctx.N * sizeof(bool), cudaMemcpyHostToDevice));
    dim3 b(16, 16), g((ctx.nx + 15) / 16, (ctx.ny + 15) / 16);
    ...
    cudaDeviceSynchronize();
    std::vector<double> gpu(ctx.N);
    cudaMemcpy(gpu.data(), dxf, ctx.N * sizeof(double), cudaMemcpyDeviceToHost);
...
static int test_dot(TestCtx& ctx) {
    ...
    double *da, *db, *dpart;
    bool* ds;
    CUDA_CHECK(cudaMalloc(&da, ctx.N * sizeof(double)));
    CUDA_CHECK(cudaMemcpy(da, ctx.b.data(), ctx.N * sizeof(double), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMalloc(&db, ctx.N * sizeof(double)));
    CUDA_CHECK(cudaMemcpy(db, ctx.b.data(), ctx.N * sizeof(double), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMalloc(&ds, ctx.N * sizeof(bool)));
    CUDA_CHECK(cudaMemcpy(ds, ctx.solid.data(), ctx.N * sizeof(bool), cudaMemcpyHostToDevice));
    int nb = (ctx.N + 255) / 256;
    CUDA_CHECK(cudaMalloc(&dpart, nb * sizeof(double)));
    kdot<<<nb, 256>>>(da, db, ds, ctx.N, dpart);
    cudaDeviceSynchronize();
...
static int test_axpy(TestCtx& ctx) {
    printf("Test 7: AXPY\n");
    std::vector<double> cpu = ctx.b, x = ctx.b;
    for (auto& v : x)
        v *= 0.5;
    ...
    double *dy, *dx;
    bool* ds;
    CUDA_CHECK(cudaMalloc(&dy, ctx.N * sizeof(double)));
    CUDA_CHECK(cudaMemcpy(dy, ctx.b.data(), ctx.N * sizeof(double), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMalloc(&dx, ctx.N * sizeof(double)));
    CUDA_CHECK(cudaMemcpy(dx, x.data(), ctx.N * sizeof(double), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMalloc(&ds, ctx.N * sizeof(bool)));
    CUDA_CHECK(cudaMemcpy(ds, ctx.solid.data(), ctx.N * sizeof(bool), cudaMemcpyHostToDevice));
    kaxpy<<<(ctx.N + 255) / 256, 256>>>(dy, dx, 0.3, ds, ctx.N);
    cudaDeviceSynchronize();
    std::vector<double> gpu(ctx.N);
    cudaMemcpy(gpu.data(), dy, ctx.N * sizeof(double), cudaMemcpyDeviceToHost);
    double d = max_diff(cpu.data(), gpu.data(), ctx.N, (bool*)ctx.solid.data());
    printf("  max|cpu-gpu|=%e  %s\n", d, d < 1e-14 ? "PASS" : "FAIL");
    cudaFree(dy);
    cudaFree(dx);
...
static int test_mean(TestCtx& ctx) {
    ...
    bool* ds;
    int* dcnt;
    CUDA_CHECK(cudaMalloc(&dv, ctx.N * sizeof(double)));
    CUDA_CHECK(cudaMemcpy(dv, ctx.b.data(), ctx.N * sizeof(double), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMalloc(&ds, ctx.N * sizeof(bool)));
    CUDA_CHECK(cudaMemcpy(ds, ctx.solid.data(), ctx.N * sizeof(bool), cudaMemcpyHostToDevice));
    int nb = (ctx.N + 255) / 256;
    CUDA_CHECK(cudaMalloc(&dpart, nb * sizeof(double)));
    CUDA_CHECK(cudaMalloc(&dcnt, nb * sizeof(int)));

    ksum<<<nb, 256>>>(dv, ds, ctx.N, dpart);
    cudaDeviceSynchronize();
    double gpu_sum = gpu_dot_reduce(dpart, nb);
    kcount<<<nb, 256>>>(ds, ctx.N, dcnt);
    cudaDeviceSynchronize();
    int gpu_cnt  = gpu_count_reduce(dcnt, nb);
    double gpu_m = gpu_cnt > 0 ? gpu_sum / gpu_cnt : 0;
    ksubmean<<<(ctx.N + 255) / 256, 256>>>(dv, gpu_m, ds, ctx.N);
    cudaDeviceSynchronize();
    std::vector<double> gpu_v(ctx.N);
    cudaMemcpy(gpu_v.data(), dv, ctx.N * sizeof(double), cudaMemcpyDeviceToHost);
    ...
    printf("  |cpu_mean-gpu_mean|=%e  max|v_cpu-v_gpu|=%e  %s\n", md, vd,
           (md < 1e-14 && vd < 1e-14) ? "PASS" : "FAIL");
    cudaFree(dv);
    cudaFree(ds);
    cudaFree(dpart);
    cudaFree(dcnt);
    return (md < 1e-14 && vd < 1e-14) ? 1 : 0;
}
...
static int test_pcg_one_iter(TestCtx& ctx) {
    ...
    // z0 = M^{-1}*r0
    cpu_rbgs_full(cz.data(), cr.data(), (bool*)ctx.solid.data(), nx, ny, stride, ctx.idx2, ctx.idy2,
                  ctx.diag);
    cp_vec        = cz;
    double crsold = cpu_dot(cr.data(), cz.data(), (bool*)ctx.solid.data(), N);
    // iter 0
    cpu_matvec(cp_vec.data(), cAp.data(), (bool*)ctx.solid.data(), nx, ny, stride, ctx.idx2,
               ctx.idy2, ctx.diag);
    ...
    double cbeta = crz / crsold;
    crsold       = crz;
    // p1 = z1 + beta * p0
    std::vector<double> cpu_p1 = cz;
    cpu_axpy(cpu_p1.data(), cp_vec.data(), cbeta, (bool*)ctx.solid.data(), N);

    // ── GPU: one PCG iteration ──
    double *dg_r, *dg_z, *dg_p, *dg_Ap, *dg_part;
    bool* dg_s;
    int* dg_cnt;
    CUDA_CHECK(cudaMalloc(&dg_r, N * sizeof(double)));
    CUDA_CHECK(cudaMemcpy(dg_r, rhs.data(), N * sizeof(double), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMalloc(&dg_z, N * sizeof(double)));
    CUDA_CHECK(cudaMemset(dg_z, 0, N * sizeof(double)));
    CUDA_CHECK(cudaMalloc(&dg_p, N * sizeof(double)));
    CUDA_CHECK(cudaMemset(dg_p, 0, N * sizeof(double)));
    CUDA_CHECK(cudaMalloc(&dg_Ap, N * sizeof(double)));
    CUDA_CHECK(cudaMalloc(&dg_s, N * sizeof(bool)));
    CUDA_CHECK(cudaMemcpy(dg_s, ctx.solid.data(), N * sizeof(bool), cudaMemcpyHostToDevice));
    ...
    // rsold = dot(r, z)
    kdot<<<nb, 256>>>(dg_r, dg_z, dg_s, N, dg_part);
    cudaDeviceSynchronize();
    ...
    cudaDeviceSynchronize();
    kdot<<<nb, 256>>>(dg_p, dg_Ap, dg_s, N, dg_part);
    cudaDeviceSynchronize();
    double gpAp = gpu_dot_reduce(dg_part, nb);

    double galpha = grsold / gpAp;
    // x += alpha*p, r -= alpha*Ap
    kaxpy<<<(N + 255) / 256, 256>>>(dg_p, dg_p, galpha - 1, dg_s,
                                    N); // hack: p was z0, now we want p = galpha*z0 ...
    // Wait, gpu p starts at 0. x += alpha * p means: x_new = 0 + alpha * p. So p should be alpha *
    // z0. Let me redo: cp = 0 + calpha * cp_vec. GPU: dg_p is p (not used as accumulator). Actually
    // dg_p is used as the accumulator. Let me restart properly.
    ...
    // p1 = z1 + beta * p0 (= dg_z already has z0, no wait — dg_z was z1. p0 is dg_z before the
    // second RBGS. But we overwrote dg_z.) We need to save p0. Let me re-read what I did above.
    // Actually: cudaMemcpy(dg_p, dg_z, ...) saved z0 as p0 (before any PCG operations on z).
    // Then I did matvec on dg_p (which is z0). Then I corrupted dg_p (memset+axpy set it to
    // alpha*z0). So p0 = dg_z at the start = z0. But dg_z was overwritten by the second RBGS. For
    // p1 = z1 + beta * p0, I need z1 (dg_z after second RBGS) and p0 (original z0, now lost). I
    // need an extra buffer. Let me use dg_Ap as temporary.
    ...
    cudaMemcpy(dg_Ap, dg_z, N * sizeof(double),
               cudaMemcpyDeviceToDevice); // dg_Ap = p0 = z0 (sacrificing dg_Ap)
    ...
    // Ap0 = matvec(p0)
    kmatvec<<<g2d, b2d>>>(dg_Ap, dg_p, dg_s, nx, ny, stride, ctx.idx2, ctx.idy2,
                          ctx.diag); // dg_p = Ap0 TEMP
    cudaDeviceSynchronize();
    kdot<<<nb, 256>>>(dg_Ap, dg_p, dg_s, N, dg_part);
    cudaDeviceSynchronize();
    ...
    // r1 = r0 - alpha * Ap0
    kaxpy<<<(N + 255) / 256, 256>>>(dg_r, dg_p, -1.0, dg_s,
                                    N); // wait, dg_p is x, not Ap0. I lost Ap0!
    ...
    CUDA_CHECK(cudaMemcpy(d_r, rhs.data(), N * sizeof(double), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMalloc(&d_z, N * sizeof(double)));
    CUDA_CHECK(cudaMemset(d_z, 0, N * sizeof(double)));
    CUDA_CHECK(cudaMalloc(&d_p0, N * sizeof(double)));
    CUDA_CHECK(cudaMalloc(&d_ap, N * sizeof(double)));

    // z0 = RBGS(r0)
    krbgs1<<<g2d, b2d>>>(d_z, d_r, dg_s, nx, ny, stride, ctx.idx2, ctx.idy2, ctx.diag);
    cudaDeviceSynchronize();
    krbgs2<<<g2d, b2d>>>(d_z, d_r, dg_s, nx, ny, stride, ctx.idx2, ctx.idy2, ctx.diag);
    cudaDeviceSynchronize();
    cudaMemcpy(d_p0, d_z, N * sizeof(double), cudaMemcpyDeviceToDevice); // p0 = z0

    kdot<<<nb, 256>>>(d_r, d_z, dg_s, N, dg_part);
    cudaDeviceSynchronize();
    grsold = gpu_dot_reduce(dg_part, nb);

    // Ap0 = A*p0
    kmatvec<<<g2d, b2d>>>(d_p0, d_ap, dg_s, nx, ny, stride, ctx.idx2, ctx.idy2, ctx.diag);
    cudaDeviceSynchronize();
    kdot<<<nb, 256>>>(d_p0, d_ap, dg_s, N, dg_part);
    cudaDeviceSynchronize();
    gpAp   = gpu_dot_reduce(dg_part, nb);
    galpha = grsold / gpAp;
    ...
    // r1 = r0 - alpha * Ap0
    kaxpy<<<(N + 255) / 256, 256>>>(d_r, d_ap, -galpha, dg_s, N);
    cudaDeviceSynchronize();

    // z1 = RBGS(r1)
    cudaMemset(d_ap, 0, N * sizeof(double)); // reuse d_ap as z1
    krbgs1<<<g2d, b2d>>>(d_ap, d_r, dg_s, nx, ny, stride, ctx.idx2, ctx.idy2, ctx.diag);
    cudaDeviceSynchronize();
    krbgs2<<<g2d, b2d>>>(d_ap, d_r, dg_s, nx, ny, stride, ctx.idx2, ctx.idy2, ctx.diag);
    cudaDeviceSynchronize();

    kdot<<<nb, 256>>>(d_r, d_ap, dg_s, N, dg_part);
    cudaDeviceSynchronize();
    ...
    kaxpy<<<(N + 255) / 256, 256>>>(d_ap, d_p0, gbeta, dg_s, N);
    cudaDeviceSynchronize(); // d_ap = z1 + beta * p0
