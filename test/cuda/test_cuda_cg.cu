...
int main() {
    ...
    for (int i = 1; i <= nx; i++)
        for (int j = 1; j <= ny; j++) {
            ...
            double pC = gpu_p[id];
            double pL =
                (i > 1 && !h_solid[(i - 1) + j * stride]) ? gpu_p[(i - 1) + j * stride] : pC;
            double pR =
                (i < nx && !h_solid[(i + 1) + j * stride]) ? gpu_p[(i + 1) + j * stride] : pC;
            double pB =
                (j > 1 && !h_solid[i + (j - 1) * stride]) ? gpu_p[i + (j - 1) * stride] : pC;
            double pT =
                (j < ny && !h_solid[i + (j + 1) * stride]) ? gpu_p[i + (j + 1) * stride] : pC;
