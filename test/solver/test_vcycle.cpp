...
int main() {
    ...
    // Test 2: restrict preserves constant
    {
        ...
        for (int i = 1; i <= 8; i++)
            for (int j = 1; j <= 8; j++)
                if (std::abs(coarse(i, j) - 5.0) > 1e-12)
                    ok = false;
    ...
    // Test 5: FMG solves manufactured problem
    {
        ...
        for (int i = 1; i <= N; i++)
            for (int j = 1; j <= N; j++)
                max_err = std::max(max_err, std::abs(s_x[0](i, j) - std::sin(M_PI * i * x.h) *
                                                                        std::sin(M_PI * j * x.h)));
